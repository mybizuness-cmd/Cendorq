...
export const CUSTOMER_PLATFORM_HANDOFF_CONTRACT = {
  ...
  handoffSurfaces: [
    ...
    surface("notifications-to-status", "Notification center", "Support status", "A support lifecycle notification has status context", "Open support status to see received, reviewing, waiting, specialist review, resolved, or closed state.", "Use safe update only when the status asks for customer input or a bounded correction.", "/dashboard/support/status"),
  ...
  safetyRules: [
    ...
    "Use defense-in-depth, least privilege, customer ownership, safe projection, auditability, secret minimization, and reduced attack surface language instead of impossible-to-hack, never-liable, guaranteed-safe, or liability-free claims.",
  ],
  recoveryRules: [
