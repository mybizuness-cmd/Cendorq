...
export const CENDORQ_PLAN_PRICES = [
  ...
  },
  {
    ...
    cadence: "one-time scoped fix",
    checkoutPath: "/checkout/start?plan=build-fix",
    successPath: "/checkout/success?plan=build-fix&session_id={CHECKOUT_SESSION_ID}",
    primaryCustomerPromise: "Improve the page, message, proof, or action path that makes the business harder to see, understand, trust, or choose.",
    revenueRole: "Turn a proven weak point into scoped implementation work.",
    afterPaymentNextStep: "Cendorq opens the Build Fix path and confirms the approved business details needed to begin.",
