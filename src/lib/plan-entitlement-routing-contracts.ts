export const PLAN_ENTITLEMENT_ROUTING_CONTRACT = {
  ...
  linearPurchaseSequences: [
    {
      ...
      notIncluded: "Deep Review report, implementation plan, done-for-you improvement, and recurring monitoring.",
      nextBestPlan: "Deep Review when the Free Scan evidence shows deeper review would materially improve decisions.",
      followUpCadence: "one report-ready email, one educational follow-up, then light suppression unless the customer re-engages or evidence-backed risk changes without pressure.",
    ...
    },
    {
      ...
      completedScope: "Deep Review report, evidence/confidence summary, priority blocker map, plan-fit rationale, and report-ready follow-up.",
      notIncluded: "Done-for-you improvement, production changes, recurring monitoring, or monthly command.",
      nextBestPlan: "Build Fix when the review identifies concrete implementation work; Ongoing Control when recurring monitoring is the clearer fit.",
    ...
    },
    {
      ...
      completedScope: "Scoped Build Fix work, implementation-ready artifacts, before-after delivery summary, approval checkpoints, and support/correction path.",
      ...
      suppressionRules: ["suppress if customer declines recurring support", "suppress if Build Fix is not yet delivered", "suppress if evidence does not support recurring control"],
    },
    {
      ...
      nextBestPlan: "Build Fix for material implementation gaps; Deep Review for standalone review clarity; expanded monthly scope only when usage and evidence support it.",
  ...
  directPurchaseWarningEmails: [
    {
      ...
      subject: "Your Build Fix scope is moving forward",
      preheader: "A quick scope note so your Build Fix work stays clear and protected.",
      bodyPurpose:
        "Confirm that the purchased Build Fix work is moving forward, explain that Deep Review is the separate review report, and remind the customer that Cendorq will use available evidence plus scoped internal orientation without delivering unpaid review artifacts.",
      allowedCopy: [
        "Your Build Fix work can continue from the scope you purchased.",
        "A full Deep Review report is a separate deliverable if you want the complete customer-facing review behind the work.",
        "Without that add-on, we will use available evidence and scoped internal orientation to support the Build Fix, and your delivered artifacts will stay within Build Fix scope.",
      ],
      blockedCopy: ["free redo", "guaranteed result", "urgent before it is too late", "we completed Deep Review for free"],
    ...
    },
    {
      ...
      bodyPurpose:
        "Confirm that monthly control is active, explain that Build Fix handles done-for-you improvement and Deep Review handles standalone deeper review, and remind the customer that monthly outputs will be based on approved scope and available evidence.",
      allowedCopy: [
        "Your Ongoing Control plan can operate from the approved monthly scope.",
        "If we identify implementation gaps, Build Fix is the separate plan for done-for-you improvement.",
        "If you want the full customer-facing review explanation, Deep Review is the separate report deliverable.",
      ],
  ...
  dashboardReminderRules: [
    ...
    "If a customer buys Build Fix without Deep Review, the dashboard may show a small recommendation to add Deep Review for the full customer-facing review while continuing the purchased Build Fix workflow.",
    "If a customer buys Ongoing Control without Build Fix, the dashboard may show evidence-backed implementation gaps and recommend Build Fix when improvement is needed.",
    "If a customer buys Ongoing Control without Deep Review, the dashboard may recommend Deep Review when a standalone deeper review would materially improve monthly command clarity.",
  ...
  entitlementBoundaries: [
    {
      ...
      notIncluded: ["Deep Review report", "competitor teardown", "implementation plan", "done-for-you improvement", "monthly monitoring"],
      internalOnlyAllowed: ["basic intake triage", "safe public signal check", "owned-surface first-read"],
    },
    {
      planKey: "deep-review",
      included: ["Deep Review report", "evidence/confidence summary", "priority blocker map", "plan-fit rationale", "report-ready follow-up"],
      notIncluded: ["done-for-you implementation", "unlimited revisions", "monthly monitoring", "production changes", "recurring improvement"],
      internalOnlyAllowed: ["implementation feasibility notes", "potential Build Fix scope hints", "Ongoing Control suitability notes"],
    },
    {
      planKey: "build-fix",
      included: ["scoped Build Fix work", "implementation-ready tasks", "copy/layout/action-path recommendations", "before-after delivery report", "approval checkpoints"],
      ...
      internalOnlyAllowed: ["minimal review orientation needed to improve safely", "internal blocker map", "internal evidence notes", "internal plan-fit review"],
    },
    {
      planKey: "ongoing-control",
      ...
      notIncluded: ["standalone Deep Review report unless purchased", "Build Fix implementation package unless purchased", "unlimited improvement", "unapproved production changes", "out-of-scope build work"],
      internalOnlyAllowed: ["monthly review orientation", "internal improvement opportunity notes", "internal risk and priority map", "internal skipped-plan recommendation basis"],
    },
  ],
  nonlinearPurchaseScenarios: [
    {
      ...
      fulfillmentRule:
        "Collect Build Fix intake and use only the minimum internal review orientation needed to perform scoped improvement safely. Do not perform or deliver the full review report unless Deep Review is purchased.",
      customerMessage:
        "We can begin the Build Fix scope you purchased. A full Deep Review report is a separate deliverable; adding it can help you understand the complete reasoning behind the work, but it is not required for us to perform the scoped improvement.",
      conversionBackPath: "Recommend Deep Review as an add-on when customer-facing review clarity would materially improve understanding, approvals, or future decisions.",
      entitlementLock: "Build Fix artifacts only; internal review orientation remains internal.",
    },
    {
      key: "ongoing-control-without-build-fix",
      customerAction: "Customer buys Ongoing Control directly without Build Fix.",
      fulfillmentRule:
        "Collect monitoring scope and use only internal monthly orientation needed for recurring command. Do not deliver a Build Fix implementation package or perform out-of-scope improvement unless Build Fix is purchased.",
      customerMessage:
    ...
    },
    {
      ...
      fulfillmentRule:
        "Use available evidence and internal orientation to operate monthly control, but do not deliver a standalone full review report unless Deep Review is purchased.",
      customerMessage:
        "We can operate monthly control from the evidence available. A full review report is recommended when you want the complete customer-facing explanation of the business gaps and priority reasoning.",
      conversionBackPath: "Recommend Deep Review when monthly command quality would benefit from a deeper customer-facing review foundation.",
      entitlementLock: "Monthly command summaries only; full review report requires Deep Review purchase.",
    },
    {
      key: "deep-review-without-build-fix",
      customerAction: "Customer buys Deep Review but declines Build Fix.",
      fulfillmentRule:
        "Deliver the review report and implementation recommendations, but do not perform done-for-you improvement work.",
      customerMessage:
        "The report can show what should change and why. Build Fix is the separate plan for doing the scoped improvement work with approval checkpoints.",
      conversionBackPath: "Recommend Build Fix only when the review report identifies concrete implementation work that the customer wants handled for them.",
      entitlementLock: "Review artifacts only; done-for-you implementation requires Build Fix purchase.",
    },
    {
      key: "free-scan-only",
      customerAction: "Customer uses Free Scan and declines paid plans.",
      fulfillmentRule:
        "Deliver the bounded free first-read report and helpful education. Do not provide Deep Review depth, implementation package, or monthly control.",
      customerMessage:
