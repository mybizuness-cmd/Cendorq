...
expect(apiPath, [
  ...
  "Billing approval requires a billing approver session.",
  "MAX_BILLING_APPROVAL_BYTES = 14_000",
  ...
  "Customer-safe billing outcome copy is required before approval.",
  "The billing approval audit record could not be created safely.",
  "The billing approval record could not be created safely.",
  "auditRecorded: true",
  "projection: \"operator-billing-approval-safe\"",
  "The billing approval storage layer could not complete the safe mutation.",
]);
...
expect(packagePath, [
  "validate:routes",
  "validate-customer-support-operator-billing-approval-api.mjs",
...
if (failures.length) {
  console.error("Customer support operator billing approval API validation failed:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log("Customer support operator billing approval API validation passed with owner posture coverage.");
