  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="relative overflow-hidden rounded-[2rem] border border-white/80 bg-white/88 p-5 shadow-[0_18px_55px_rgba(15,23,42,0.055)] backdrop-blur sm:p-6">
      ...
      <div className="relative">
        <h2 className="text-3xl font-semibold leading-tight tracking-[-0.055em] text-slate-950">Update a waiting support request safely.</h2>
        <p className="mt-3 text-sm font-semibold leading-7 text-slate-600">{UPDATE_WARNING}</p>
        <div className="mt-5 rounded-[1.25rem] border border-slate-200 bg-white/88 p-4 text-xs font-semibold leading-6 text-slate-600 shadow-sm">
        ...
        {state.kind === "success" ? <div className="mt-6 rounded-[1.5rem] border border-emerald-200 bg-emerald-50 p-5"><div className="text-sm font-semibold text-emerald-950">{state.response.message}</div><div className="mt-3 grid gap-2 text-xs font-semibold leading-6 text-emerald-900 sm:grid-cols-2"><div>Request ID: {state.response.supportRequestId}</div><div>Status: {state.response.status}</div><div>Raw payload stored: {state.response.rawPayloadStored ? "yes" : "no"}</div><div>Projection: {state.response.customerSafeProjectionOnly ? "customer-safe only" : "not available"}</div></div><div className="mt-5 flex flex-col gap-3 sm:flex-row"><Link href="/dashboard/support/status" className={BUTTON_PRIMARY}>Track updated request</Link><Link href="/dashboard/support" className={BUTTON_SECONDARY}>Back to support center</Link></div></div> : null}
        ...
        <div className="sr-only">Light support update form. No black support update blocks. No dark blue support update blocks. Badge styling removed from visible support update blocks. Heavy blue blocks reduced.</div>
      </div>
    </form>
  );
}
