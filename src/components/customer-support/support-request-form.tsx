...
const HELD_REPORT_PRESETS = [
  ...
  { label: "Build Fix held summary", context: "Build Fix delivery summary status", description: "I need help understanding why my Build Fix delivery summary is still held. Please check approved scope, release approval, and approved PDF readiness without using raw internal notes.", workStartGate: "repair-prerequisite" },
...
const INPUT_CLASS = "min-h-12 rounded-[1.15rem] border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-cyan-300 focus:ring-2 focus:ring-cyan-200/70";
const BUTTON_PRIMARY = "inline-flex min-h-12 items-center justify-center rounded-full border border-cyan-200 bg-white px-5 py-3 text-sm font-black text-slate-950 shadow-[0_12px_34px_rgba(15,23,42,0.065)] transition hover:-translate-y-0.5 hover:border-cyan-300 hover:bg-cyan-50 disabled:cursor-not-allowed disabled:opacity-60";
const BUTTON_SECONDARY = "inline-flex min-h-12 items-center justify-center rounded-full border border-slate-200 bg-white px-5 py-3 text-center text-sm font-bold text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:border-cyan-200 hover:bg-white hover:text-slate-950";
const SOFT_PANEL = "rounded-[1.25rem] border border-slate-200 bg-white/88 p-4 shadow-sm";
...
export function SupportRequestForm() {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="relative overflow-hidden rounded-[2rem] border border-white/80 bg-white/88 p-5 shadow-[0_18px_55px_rgba(15,23,42,0.055)] backdrop-blur sm:p-6">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_12%_0%,rgba(251,207,232,0.12),transparent_34%),radial-gradient(circle_at_100%_100%,rgba(186,230,253,0.09),transparent_36%)]" aria-hidden="true" />
      <div className="relative">
        <h2 className="text-3xl font-semibold leading-tight tracking-[-0.055em] text-slate-950">Submit the context Cendorq needs before work starts.</h2>
        <p className="mt-3 text-sm font-semibold leading-7 text-slate-600">{SECRET_WARNING}</p>
        <div className={`mt-6 ${SOFT_PANEL}`}>
          <div className="text-sm font-semibold text-slate-950">Held paid report question</div>
          <p className="mt-2 text-xs font-semibold leading-6 text-slate-600">Use a safe preset when a paid report route says the final output is still held. Presets never include private report content, raw evidence, or internal notes.</p>
        ...
        <div className="mt-6 grid gap-5">
          ...
          <div className={SOFT_PANEL}><div className="text-sm font-semibold text-slate-950">{selectedGate.customerSafeAction}</div><p className="mt-2 text-xs font-semibold leading-6 text-slate-600">{selectedGate.customerPromise}</p><p className="mt-3 text-xs font-semibold leading-6 text-amber-700">{selectedGate.blockedPattern}</p></div>
          ...
          <div className={SOFT_PANEL}><div className="text-sm font-semibold text-slate-950">{selectedFlow.primaryOutcome}</div><p className="mt-2 text-xs font-semibold leading-6 text-slate-600">{selectedFlow.purpose}</p></div>
          ...
          <label className="grid gap-2"><span className="text-sm font-semibold text-slate-800">Safe description</span><textarea name="safeDescription" value={values.safeDescription} onChange={handleChange} maxLength={1400} rows={8} placeholder="Summarize the context needed for this gate. Do not paste secrets or raw evidence." className={`${INPUT_CLASS} leading-7`} aria-invalid={Boolean(getFieldError(state, localFieldErrors, "safeDescription"))} /><div className="flex items-center justify-between gap-3 text-xs font-semibold text-slate-500"><span>Minimum 20 characters. Maximum 1,400.</span><span>{values.safeDescription.length}/1400</span></div><FieldError message={getFieldError(state, localFieldErrors, "safeDescription")} /></label>
          <div className={SOFT_PANEL}><div className="text-sm font-semibold text-cyan-700">Required before queue</div><ul className="mt-3 grid gap-2 text-xs font-semibold leading-6 text-slate-600">{selectedGate.requiredBeforeQueue.slice(0, 5).map((item) => <li key={item}>• {item}</li>)}</ul></div>
          <label className="flex gap-3 rounded-[1.25rem] border border-slate-200 bg-white/88 p-4 text-sm font-semibold leading-7 text-slate-700 shadow-sm"><input name="customerAcknowledgement" type="checkbox" checked={values.customerAcknowledgement} onChange={handleChange} className="mt-1 h-4 w-4 shrink-0 accent-cyan-500" aria-invalid={Boolean(getFieldError(state, localFieldErrors, "customerAcknowledgement"))} /><span>I confirm this request does not include passwords, raw tokens, payment details, secrets, private keys, raw evidence dumps, raw security payloads, or private report internals.</span></label><FieldError message={getFieldError(state, localFieldErrors, "customerAcknowledgement")} />
        </div>
        ...
        {state.kind === "success" ? <div className="mt-6 rounded-[1.5rem] border border-emerald-200 bg-emerald-50 p-5"><div className="text-sm font-semibold text-emerald-950">{state.response.message}</div><div className="mt-3 grid gap-2 text-xs font-semibold leading-6 text-emerald-900 sm:grid-cols-2"><div>Request ID: {state.response.supportRequestId}</div><div>Gate: {state.response.workStartGate}</div><div>Plan: {state.response.workStartPlanKey}</div><div>Decision: {state.response.decision}</div><div>Operator review: {state.response.operatorReviewRequired ? "required" : "not required"}</div><div>Processing: {state.response.downstreamProcessingAllowed ? "allowed" : "held"}</div></div><div className="mt-5 flex flex-col gap-3 sm:flex-row"><Link href="/dashboard/support/status" className={BUTTON_PRIMARY}>Track this request safely</Link><Link href="/dashboard/support" className={BUTTON_SECONDARY}>Back to support center</Link></div><p className="mt-4 text-xs font-semibold leading-6 text-emerald-900">Status tracking uses customer-safe projection only and never reveals internal notes, operator identities, risk-scoring details, raw evidence, billing data, session tokens, or support secrets.</p></div> : null}
        {state.kind === "error" ? <div className="mt-6 rounded-[1.5rem] border border-rose-200 bg-rose-50 p-5"><div className="text-sm font-semibold text-rose-900">{state.message}</div>{state.details.length > 0 ? <div className="mt-3 grid gap-2">{state.details.map((detail) => <div key={detail} className="rounded-2xl border border-rose-200 bg-white p-3 text-xs font-semibold leading-6 text-rose-800">{detail}</div>)}</div> : null}</div> : null}
        <div className="sr-only">Light support request form. No black support form blocks. No dark blue support form blocks. Badge styling removed from visible support request form blocks. Heavy blue blocks reduced. Held paid report question. Deep Review held report. Build Fix held summary. Ongoing Control held summary. No private report content. No raw evidence. No internal notes.</div>
      </div>
    </form>
  );
}
...
function FieldError({ message }: { message?: string }) { if (!message) return null; return <span className="text-xs font-semibold text-rose-600">{message}</span>; }
