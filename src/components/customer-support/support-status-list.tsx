...
const BUTTON_PRIMARY = "inline-flex min-h-12 items-center justify-center rounded-full border border-cyan-200 bg-white px-5 py-3 text-sm font-black text-slate-950 shadow-[0_12px_34px_rgba(15,23,42,0.065)] transition hover:-translate-y-0.5 hover:border-cyan-300 hover:bg-cyan-50";
...
export function SupportStatusList() {
  ...
  return (
    <section className="relative overflow-hidden rounded-[2rem] border border-white/80 bg-white/88 p-5 shadow-[0_18px_55px_rgba(15,23,42,0.055)] backdrop-blur sm:p-6">
      ...
      <div className="relative">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h2 className="text-3xl font-semibold leading-tight tracking-[-0.055em] text-slate-950">Your protected support requests.</h2>
        ...
        {state.kind === "loading" ? <div className="mt-6 rounded-[1.5rem] border border-slate-200 bg-white/88 p-5 text-sm font-semibold leading-7 text-slate-600 shadow-sm">Loading protected support status...</div> : null}

        {state.kind === "empty" ? (
          <div className="mt-6 rounded-[1.5rem] border border-slate-200 bg-white/88 p-5 shadow-sm">
            <div className="text-sm font-semibold text-slate-950">No active support requests are visible yet.</div>
            <p className="mt-2 text-sm font-semibold leading-7 text-slate-600">Start a protected request when you need report help, correction review, billing support, security review, or plan guidance.</p>
        ...
        {state.kind === "error" ? (
          <div className="mt-6 rounded-[1.5rem] border border-rose-200 bg-rose-50 p-5">
            ...
            {state.details.length ? <div className="mt-3 grid gap-2">{state.details.map((detail) => <div key={detail} className="rounded-2xl border border-rose-200 bg-white p-3 text-xs font-semibold leading-6 text-rose-800">{detail}</div>)}</div> : null}
          </div>
        ) : null}
        ...
        {state.kind === "ready" ? (
          <div className="mt-6 grid gap-4">
            {state.entries.map((entry) => {
              ...
              return (
                <article key={entry.supportRequestId} className="rounded-[1.5rem] border border-slate-200 bg-white/88 p-5 shadow-sm">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      <div className="text-sm font-semibold text-cyan-700">{entry.statusLabel}</div>
                      <h3 className="mt-3 text-2xl font-semibold tracking-[-0.045em] text-slate-950">{entry.businessContext}</h3>
                      <p className="mt-3 text-sm font-semibold leading-7 text-slate-600">{entry.statusCopy}</p>
                    </div>
                  ...
                  <div className={`${SOFT_PANEL} mt-4 text-xs font-semibold leading-6 text-slate-600`}>
        ...
        <div className="sr-only">Light support status list. No black support status list blocks. No dark blue support status list blocks. Badge styling removed from visible support status list blocks. Heavy blue blocks reduced.</div>
      </div>
    </section>
  );
}
...
function WorkStartGatePanel({ entry }: { entry: CustomerSupportStatusEntry }) {
  ...
  return <div className={`${SOFT_PANEL} mt-4`}><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><div className="text-sm font-semibold text-cyan-700">Cendorq work-start gate</div><p className="mt-2 text-sm font-semibold text-slate-950">{entry.workStartGateTitle}</p><p className="mt-2 max-w-3xl text-xs font-semibold leading-6 text-slate-600">{entry.workStartCustomerAction}</p></div><Link href={buildGatePath(entry)} className={BUTTON_SECONDARY}>Open gate path</Link></div><div className="mt-4 grid gap-3 md:grid-cols-3"><StatusDetail label="Plan layer" value={entry.workStartPlanKey} /><StatusDetail label="Required before queue" value={required} /><StatusDetail label="Blocked pattern" value={entry.workStartBlockedPattern} /></div></div>;
}
...
function CommunicationPlanPanel({ communicationPlan, supportRequestId }: { communicationPlan: CustomerSupportCommunicationPlan; supportRequestId: string }) {
  ...
  return <div className={`${SOFT_PANEL} mt-4`}><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><div className="text-sm font-semibold text-cyan-700">Next communication path</div><p className="mt-2 text-sm font-semibold text-slate-950">{formatCommunicationDecision(communicationPlan.decision)}</p><p className="mt-2 max-w-3xl text-xs font-semibold leading-6 text-slate-600">{communicationDecisionCopy(communicationPlan.decision)}</p></div><Link href={communicationPath} className={BUTTON_SECONDARY}>Open communication path</Link></div><div className="mt-4 grid gap-3 md:grid-cols-3"><StatusDetail label="Communication decision" value={formatCommunicationDecision(communicationPlan.decision)} /><StatusDetail label="Safe channels" value={channels} /><StatusDetail label="Required guards" value={guards} /></div></div>;
}

function StatusDetail({ label, value }: { label: string; value: string }) { return <div className="rounded-[1rem] border border-slate-200 bg-white/88 p-4 shadow-sm"><div className="text-xs font-semibold text-slate-500">{label}</div><div className="mt-2 break-words text-sm font-semibold text-slate-950">{value}</div></div>; }
