const PREVIEW_PILLARS = [
  { label: "Findability", score: 58, copy: "Can public systems locate the business and its signals?" },
  { label: "Understanding", score: 39, copy: "Can the offer, audience, and location be understood quickly?" },
  { label: "Trust", score: 44, copy: "Is proof visible near the decision point?" },
  { label: "Choice", score: 31, copy: "Is there a clear reason to choose this business?" },
  { label: "Action", score: 52, copy: "Is the next customer step obvious?" },
] as const;
        ...
        <div className="rounded-[1.75rem] border border-slate-200 bg-[radial-gradient(circle_at_48%_0%,#ffffff_0%,#effcff_42%,#dff4ff_100%)] p-5 shadow-inner sm:rounded-[2.35rem] sm:p-6">
          ...
          <div className="mt-5 rounded-[1.4rem] border border-slate-200 bg-slate-950 p-5 text-white shadow-[0_18px_55px_rgba(15,23,42,0.18)]">
            ...
            <div className="mt-4 grid gap-3">
              {PREVIEW_REPAIRS.map((item, index) => (
                <div key={item} className="flex gap-3 rounded-[1rem] border border-white/10 bg-white/7 p-3">
