...
export function PresenceReportEvidenceBoundaryPanel() {
  return (
    <section className="rounded-[2rem] border border-white/80 bg-white/84 p-6 shadow-[0_18px_60px_rgba(14,165,233,0.065)] backdrop-blur sm:rounded-[2.5rem] sm:p-8" aria-label="Presence Report evidence boundaries">
      <div className="grid gap-5 lg:grid-cols-[0.72fr_1.28fr] lg:items-end">
        <h2 className="text-[clamp(2rem,6vw,3.5rem)] font-semibold leading-[0.98] tracking-[-0.065em] text-slate-950">Useful signal without overclaiming.</h2>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        {SAMPLE_PRESENCE_REPORT_EVIDENCE_BOUNDARIES.map((item) => (
          <article key={item.label} className="rounded-[1.35rem] border border-cyan-100 bg-cyan-50/38 p-5 shadow-sm">
            <h3 className="text-xl font-semibold tracking-[-0.04em] text-slate-950">{item.label}</h3>
            <p className="mt-2 text-xs font-bold text-cyan-700">{item.confidence}</p>
      ...
      <span className="sr-only">Evidence boundaries. publicSafeUse. boundary.</span>
    </section>
  );
}
