const ASSETS = [
  ["Presence Gap Index", "Benchmark answer-readiness by vertical instead of pretending to be a broad SEO index."],
  ["Vertical proof sets", "Show what different business categories need to be understood and chosen."],
  ["Repair library", "Turn repeated weak signals into clearer page, proof, FAQ, and action patterns."],
] as const;
    ...
    <section className="relative px-5 py-7 sm:px-8 lg:py-10" aria-label="Authority proof">
      <div className="mx-auto max-w-7xl rounded-[2.3rem] border border-white/80 bg-white/86 p-6 shadow-[0_22px_80px_rgba(15,23,42,0.075)] backdrop-blur sm:rounded-[3rem] sm:p-8 lg:p-10">
        ...
        <div className="mt-7 grid gap-4 lg:grid-cols-3">
          {ASSETS.map(([label, copy]) => (
            <article key={label} className="rounded-[1.5rem] border border-cyan-100 bg-cyan-50/55 p-5">
              <h3 className="text-2xl font-semibold tracking-[-0.05em] text-slate-950">{label}</h3>
