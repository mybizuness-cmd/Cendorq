...
export function HomepageClarityReset() {
  return (
    <main
      ...
      className="min-h-screen bg-[radial-gradient(circle_at_8%_0%,rgba(251,207,232,0.34),transparent_30%),radial-gradient(circle_at_96%_8%,rgba(125,211,252,0.42),transparent_36%),linear-gradient(180deg,#fffaff_0%,#eefbff_34%,#ffffff_100%)] text-slate-950"
    >
        ...
        <div className="mx-auto grid max-w-7xl gap-9 lg:min-h-[calc(100vh-5.5rem)] lg:grid-cols-[0.82fr_1.18fr] lg:items-center">
          <div>
            ...
            <p className="mt-6 max-w-2xl text-lg font-semibold leading-8 text-slate-600 sm:text-xl sm:leading-9">
              Cendorq scans the public signals around your business, shows the first weak point, and turns it into one clear repair path.
            </p>
      ...
      <section className="mx-auto grid max-w-7xl gap-4 px-5 pb-14 sm:px-8 md:grid-cols-3" aria-label="Cendorq path">
        {[
          ["Scan", "Find the first weak public signal."],
          ["Review", "Understand why customers or AI systems may hesitate."],
          ["Repair", "Fix the clearest page, proof, message, or action path."],
        ].map(([label, copy]) => (
          <article key={label} className="rounded-[1.4rem] border border-cyan-100 bg-white/84 p-5 shadow-[0_14px_38px_rgba(14,165,233,0.07)]">
