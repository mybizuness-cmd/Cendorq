...
const RAILS = [
  ["Surface", "what buyers see"],
  ["Meaning", "what AI can explain"],
...
export function HomepageClarityReset() {
  return (
    <main data-cendorq-homepage="final-master-presence-product-film" className="group/home relative isolate min-h-screen overflow-x-hidden bg-[#040b18] text-white">
      ...
      <div className="pointer-events-none absolute left-1/2 top-[8rem] -z-10 h-[28rem] w-[70rem] -translate-x-1/2 rounded-[8rem] bg-cyan-300/10 blur-3xl" />
...
function SignalEngine() {
  return (
    <div className="relative mx-auto flex min-h-[43rem] w-full max-w-[64rem] items-center justify-center [perspective:1800px]">
      <div className="absolute left-[4%] top-[8%] h-32 w-32 rounded-[2.6rem] border border-cyan-200/20 bg-cyan-200/10 blur-sm motion-safe:animate-pulse" />
      <div className="absolute bottom-[6%] right-[4%] h-44 w-44 rounded-[3.4rem] border border-violet-200/20 bg-violet-300/10 blur-sm motion-safe:animate-pulse" />
      <div className="presence-orbit absolute h-[34rem] w-[34rem] rounded-[5.4rem] border border-cyan-200/22 bg-cyan-100/5 shadow-[0_0_130px_rgba(34,211,238,.17)] motion-safe:animate-[spin_18s_linear_infinite]" />
      <div className="presence-orbit absolute h-[26rem] w-[26rem] rounded-[4.2rem] border border-violet-200/20 bg-violet-100/5 shadow-[0_0_100px_rgba(167,139,250,.14)] motion-safe:animate-[spin_24s_linear_infinite_reverse]" />
      ...
      <div className="presence-card relative w-full max-w-[48rem] overflow-hidden rounded-[2.8rem] border border-white/18 bg-white/[.10] p-5 shadow-[0_56px_170px_rgba(0,0,0,.46),inset_0_1px_0_rgba(255,255,255,.22)] backdrop-blur-2xl transition duration-700 hover:-translate-y-3 hover:rotate-1 sm:p-6">
        ...
        <div className="relative mt-5 overflow-hidden rounded-[2.15rem] border border-cyan-100/18 bg-slate-950/84 p-5 shadow-[0_28px_96px_rgba(0,0,0,.38)] sm:p-6">
          <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-cyan-300/80 to-transparent" />
          <div className="grid gap-5 lg:grid-cols-[.66fr_1.34fr] lg:items-center">
            <div>
...
function SystemCore() {
  return (
    <div className="relative min-h-72 overflow-hidden rounded-[1.8rem] border border-white/10 bg-white/[.06] p-4">
      ...
      <div className="absolute inset-x-[18%] top-1/2 h-px bg-gradient-to-r from-cyan-200/0 via-cyan-200/55 to-cyan-200/0" />
      <div className="absolute left-1/2 inset-y-[22%] w-px bg-gradient-to-b from-violet-200/0 via-violet-200/45 to-violet-200/0" />
      <div className="absolute bottom-4 left-4 right-4 grid grid-cols-3 gap-2">
        {RAILS.map(([title, copy]) => <Rail key={title} title={title} copy={copy} />)}
      </div>
    </div>
  );
}
