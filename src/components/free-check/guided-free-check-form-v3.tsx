  ...
  return (
    <section id="free-check-intake" className={className}>
      <div className="rounded-[2.25rem] border border-slate-200 bg-white p-5 shadow-[0_30px_100px_rgba(15,23,42,0.11)] sm:p-7">
        ...
        <div className="sr-only">Free Scan form v3 preserves API payload, validation, signal quality, routing hint, verify-to-view handoff, dashboard Free Scan result path, account continuation standard, inbox guidance, clean public CTAs, recovery guidance, safe-data warnings, light cyan progress bar, device progress events, submitted marker, and no black form buttons. First-use progress starts at zero until the customer types.</div>
      </div>
    </section>
  );
}
...
function successNextMove(routingHint: RoutingHint) { if (routingHint === "command-review") return { href: "/plans/ongoing-control", cta: "See Readiness Control" }; if (routingHint === "infrastructure-review") return { href: "/plans/build-fix", cta: "See Signal Repair" }; if (routingHint === "blueprint-candidate") return { href: "/plans/deep-review", cta: "See AI Readiness Review" }; return { href: "/plans", cta: "Compare plans" }; }
