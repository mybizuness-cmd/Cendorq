...
const SUCCESS_NEXT_STEPS = ["Your answers were received.", "The scan email becomes the workspace email.", "Your protected result opens in the customer workspace."] as const;
...
export function GuidedFreeCheckFormV3({ className }: { className?: string }) {
  ...
  useEffect(() => { recordFreeScanProgress(values); }, [values]);
    ...
    <section id="free-check-intake" className={className}>
      <div className="rounded-[2.25rem] border border-slate-200 bg-white p-5 shadow-[0_30px_100px_rgba(15,23,42,0.11)] sm:p-7">
        ...
        <form className="mt-6" onSubmit={submit} noValidate>
          ...
          <div className="mt-6 flex flex-col gap-4 border-t border-slate-200 pt-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-wrap gap-3">{step > 0 ? <button type="button" onClick={() => setStep((current) => Math.max(0, current - 1) as StepNumber)} className={SECONDARY_CTA_CLASS}>Back</button> : null}{step < STEPS.length - 1 ? <button type="button" onClick={nextStep} className={PRIMARY_CTA_CLASS}>Continue</button> : <button type="submit" disabled={isSubmitting} className={PRIMARY_CTA_CLASS}>{isSubmitting ? "Submitting scan..." : "Submit Free Scan"}</button>}</div><div className="text-sm leading-6 text-slate-500">Use business context only. Do not enter private credentials.</div></div>
          {submitState.kind === "success" ? <SuccessState state={submitState} /> : null}{submitState.kind === "error" ? <ErrorState state={submitState} /> : null}
        </form>
        <div className="sr-only">Free Scan form v3 preserves API payload, validation, signal quality, routing hint, verify-to-view handoff, dashboard Free Scan result path, account continuation standard, inbox guidance, clean public CTAs, recovery guidance, safe-data warnings, light cyan progress bar, device progress events, submitted marker, and no black form buttons.</div>
      </div>
    </section>
  );
}

function FieldControl({ field, values, errors, onChange }: { field: keyof FormValues; values: FormValues; errors: Partial<Record<keyof FormValues, string>>; onChange: (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => void }) { const copy = LABELS[field]; const wide = ["audience", "biggestIssue", "competitors", "notes"].includes(field); return <div className={wide ? "md:col-span-2" : undefined}><label htmlFor={field} className="mb-2 block text-sm font-semibold text-slate-800">{copy.label}</label>{renderInput(field, values, onChange)}<p className="mt-2 text-xs leading-5 text-slate-500">{copy.helper}</p>{errors[field] ? <p className="mt-2 text-sm font-semibold text-rose-600">{errors[field]}</p> : null}</div>; }
...
function successNextMove(routingHint: RoutingHint) { if (routingHint === "command-review") return { title: "Readiness Control may fit later.", copy: "Use it when the business needs recurring direction, monitoring, and monthly decision support.", href: "/plans/ongoing-control", cta: "See Readiness Control" }; if (routingHint === "infrastructure-review") return { title: "Signal Repair may fit later.", copy: "Use it when a specific weak point is ready for scoped improvement.", href: "/plans/build-fix", cta: "See Signal Repair" }; if (routingHint === "blueprint-candidate") return { title: "AI Readiness Review may be the right next depth.", copy: "Use it when the scan shows you need evidence-backed review before paying for repair.", href: "/plans/deep-review", cta: "See AI Readiness Review" }; return { title: "Start with the first signal before spending deeper.", copy: "The Free Scan keeps the next move safer when the business still needs clear context.", href: "/plans", cta: "Compare plans" }; }

function readSavedFreeScanProgress() { if (typeof window === "undefined") return INITIAL_VALUES; try { const raw = window.localStorage.getItem(FREE_SCAN_PROGRESS_KEY); if (!raw) return INITIAL_VALUES; const stored = JSON.parse(raw) as { values?: Partial<FormValues> }; return { ...INITIAL_VALUES, ...sanitizeFormValues(stored.values || {}) }; } catch { return INITIAL_VALUES; } }
function recordFreeScanProgress(values: FormValues) { if (typeof window === "undefined") return; try { const hasStarted = Object.values(values).some((value) => value.trim().length > 0); if (!hasStarted) { window.localStorage.removeItem(FREE_SCAN_PROGRESS_KEY); window.dispatchEvent(new Event("cendorq:free-check:progress")); return; } window.localStorage.setItem(FREE_SCAN_PROGRESS_KEY, JSON.stringify({ savedAt: new Date().toISOString(), values })); window.dispatchEvent(new Event("cendorq:free-check:progress")); } catch { window.dispatchEvent(new Event("cendorq:free-check:progress")); } }
function recordFreeScanSubmitted({ intakeId, routingHint }: { intakeId: string; routingHint: RoutingHint }) { if (typeof window === "undefined") return; try { window.localStorage.removeItem(FREE_SCAN_PROGRESS_KEY); window.localStorage.setItem(FREE_SCAN_SUBMITTED_KEY, JSON.stringify({ submittedAt: new Date().toISOString(), intakeId, routingHint })); window.dispatchEvent(new Event("cendorq:free-check:submitted")); } catch { window.dispatchEvent(new Event("cendorq:free-check:submitted")); } }
function sanitizeFormValues(values: Partial<FormValues>) { return (Object.keys(INITIAL_VALUES) as Array<keyof FormValues>).reduce<Partial<FormValues>>((safeValues, key) => { const value = values[key]; if (typeof value === "string") safeValues[key] = value.slice(0, 2000); return safeValues; }, {}); }
