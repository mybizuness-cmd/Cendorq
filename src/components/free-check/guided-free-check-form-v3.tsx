...
const STEPS: readonly Step[] = [
  { title: "Start the Free Scan.", copy: "Give us the business and email so the result can be saved to the right account and opened after verification.", fields: ["businessName", "websiteUrl", "fullName", "email"] },
...
const SIGNAL_PREVIEW = [
  ["Reads", "website, offer, location, proof, reviews, and action"],
  ["Checks", "findability, understanding, trust, choice, and next step"],
  ["Returns", "first signal, likely gap, and safest next command"],
] as const;
...
export function GuidedFreeCheckFormV3({ className }: { className?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/free-check", {
        ...
        body: JSON.stringify({ ...values, email: submittedEmail, websiteUrl: normalizeWebsite(values.websiteUrl), country: locationParts.country, stateRegion: locationParts.stateRegion, city: locationParts.city, competitors: "", notes: "", source: "free-check" }),
      });
  ...
  return (
    <section id="free-check-intake" className={className}>
      <div className="relative overflow-hidden rounded-[2.25rem] border border-white/80 bg-white/88 p-2 shadow-[0_30px_100px_rgba(15,23,42,0.11)] backdrop-blur-2xl">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_10%_0%,rgba(251,207,232,0.14),transparent_32%),radial-gradient(circle_at_100%_10%,rgba(186,230,253,0.14),transparent_34%)]" aria-hidden="true" />
        <div className="relative rounded-[1.85rem] border border-slate-200/80 bg-white p-5 sm:p-7">
          <div className="grid gap-4 border-b border-slate-200 pb-5 lg:grid-cols-[1fr_16rem] lg:items-start">
            <div>
              <p className="text-sm font-semibold text-cyan-700">Free Scan intake</p>
              <h2 className="mt-3 max-w-3xl text-3xl font-semibold leading-tight tracking-[-0.06em] text-slate-950 sm:text-4xl">{activeStep.title}</h2>
              <p className="mt-3 max-w-3xl text-sm font-semibold leading-7 text-slate-600">{activeStep.copy}</p>
            </div>
            <div className="rounded-[1.25rem] border border-slate-200 bg-white px-4 py-3 shadow-sm">
              ...
              <div className="mt-1 text-3xl font-semibold tracking-[-0.07em] text-slate-950">{qualityScore}%</div>
              <div className="mt-1 text-xs font-semibold leading-5 text-slate-500">Clear answers improve the first signal.</div>
            </div>
          </div>

          <div className="mt-5 grid gap-3 md:grid-cols-3">
            {SIGNAL_PREVIEW.map(([label, copy]) => (
              <div key={label} className="rounded-[1.1rem] border border-slate-200 bg-white/92 p-4 shadow-sm">
          ...
          <div className="mt-5">
            ...
            <div className="mt-2 h-2 overflow-hidden rounded-full bg-cyan-50"><span className="block h-full rounded-full bg-[linear-gradient(90deg,#bae6fd,#f5d0fe)] transition-all" style={{ width: `${progress}%` }} /></div>
          </div>
          ...
          <form className="mt-6" onSubmit={submit} noValidate>
            ...
            <div className="mt-6 flex flex-col gap-4 border-t border-slate-200 pt-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex flex-wrap gap-3">{step > 0 ? <button type="button" onClick={() => setStep(0)} className={SECONDARY_CTA_CLASS}>Back</button> : null}{step < STEPS.length - 1 ? <button type="button" onClick={nextStep} className={PRIMARY_CTA_CLASS}>Continue</button> : <button type="submit" disabled={isSubmitting} className={PRIMARY_CTA_CLASS}>{isSubmitting ? "Submitting scan..." : "Submit Free Scan"}</button>}</div>
              <div className="rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-500 shadow-sm">Use business context only. Do not enter private credentials.</div>
            </div>
...
function FieldControl({ field, values, errors, onChange }: { field: keyof FormValues; values: FormValues; errors: Partial<Record<keyof FormValues, string>>; onChange: (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => void }) {
  ...
  const wide = ["primaryOffer", "audience", "biggestIssue"].includes(field);
  return (
    <div className={wide ? "md:col-span-2" : undefined}>
      <label htmlFor={field} className="mb-2 block text-sm font-semibold text-slate-800">{copy.label}</label>
      {renderInput(field, values, onChange)}
      <p className="mt-2 text-xs font-semibold leading-5 text-slate-500">{copy.helper}</p>
      {errors[field] ? <p className="mt-2 text-sm font-semibold text-rose-600">{errors[field]}</p> : null}
    </div>
  );
}
...
function renderInput(field: keyof FormValues, values: FormValues, onChange: (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => void) {
  const commonClass = "w-full rounded-[1.05rem] border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-cyan-300 focus:ring-2 focus:ring-cyan-200/70";
  const copy = LABELS[field];
...
function SuccessState({ state }: { state: Extract<SubmitState, { kind: "success" }> }) {
  ...
  return <div className="mt-6 overflow-hidden rounded-[1.5rem] border border-emerald-200 bg-emerald-50 p-5 sm:p-6"><div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-emerald-700">Scan received</div><h3 className="mt-3 text-2xl font-semibold tracking-[-0.04em] text-slate-950">Confirm your email to open the result.</h3><p className="mt-3 text-sm font-semibold leading-7 text-slate-700">{state.message}</p><div className="mt-4 rounded-[1.25rem] border border-emerald-200 bg-white p-4"><div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-emerald-700">Account access</div><p className="mt-2 text-sm font-semibold leading-7 text-slate-700">{state.accountMessage}</p><p className="mt-2 text-xs font-semibold leading-6 text-slate-500">{state.accountOperatorMessage}</p></div><div className="mt-5 grid gap-3 md:grid-cols-3">{SUCCESS_NEXT_STEPS.map((item) => <div key={item} className="rounded-[1.1rem] border border-emerald-200 bg-white px-4 py-4 text-sm font-semibold leading-6 text-slate-700">{item}</div>)}</div><div className="mt-5 rounded-[1.25rem] border border-slate-200 bg-white p-4"><div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400">After verification</div><p className="mt-2 text-sm font-semibold leading-7 text-slate-700">{handoff.safeCustomerMessage}</p><p className="mt-2 text-xs font-semibold leading-6 text-slate-500">{handoff.reportVisibilityRule}</p></div><div className="mt-5 flex flex-col gap-3 sm:flex-row"><Link href={handoff.verifiedDestination} className={PRIMARY_CTA_CLASS}>{handoff.primaryCta}</Link><Link href={nextMove.href} className={SECONDARY_CTA_CLASS}>{nextMove.cta}</Link><Link href="/plans" className={SECONDARY_CTA_CLASS}>Compare plans</Link></div></div>;
}

function ErrorState({ state }: { state: Extract<SubmitState, { kind: "error" }> }) { return <div className="mt-6 rounded-[1.5rem] border border-rose-200 bg-rose-50 p-5 sm:p-6"><div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-rose-700">Scan not sent yet</div><h3 className="mt-3 text-2xl font-semibold tracking-[-0.04em] text-slate-950">Nothing is lost. Fix the issue and send it again.</h3><p className="mt-3 text-sm font-semibold leading-7 text-slate-700">{state.message}</p>{state.details.length ? <div className="mt-4 grid gap-2">{state.details.map((detail) => <div key={detail} className="rounded-[1rem] border border-rose-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">{detail}</div>)}</div> : null}<div className="mt-5 grid gap-3 md:grid-cols-3">{RECOVERY_TILES.map((item) => <div key={item} className="rounded-[1.1rem] border border-rose-200 bg-white px-4 py-4 text-sm font-semibold leading-6 text-slate-700">{item}</div>)}</div></div>; }
