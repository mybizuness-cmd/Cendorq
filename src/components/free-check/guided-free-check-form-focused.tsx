...
type SubmitState =
  | { kind: "idle" }
  | { kind: "success"; message: string; routingHint: RoutingHint; handoff: FreeScanVerifyToViewClientHandoff }
...
const STEPS: readonly Step[] = [
  {
    eyebrow: "Business seen by customers",
    title: "Tell us what customers see first.",
    copy: "Use the business name, website, and contact tied to this scan. This is the base for the first visibility and trust read.",
    fields: ["businessName", "websiteUrl", "fullName", "email"],
  },
  {
    eyebrow: "Market context",
    title: "Place the business where buyers compare it.",
    copy: "Location and category change how people, maps, reviews, and AI/search systems understand the business.",
    fields: ["country", "stateRegion", "city", "businessType"],
  },
  {
    eyebrow: "Offer clarity",
    title: "Say what you sell and who should choose it.",
    copy: "Plain language helps Cendorq see whether the business is easy to understand, trust, compare, and act on.",
    fields: ["primaryOffer", "audience"],
  },
  {
    eyebrow: "Decision break",
    title: "Name what is making people hesitate.",
    copy: "Tell us what feels weak, confusing, or harder than it should be. This is where the first read gets useful.",
...
const LABELS: Record<keyof FormValues, { label: string; helper: string; placeholder: string }> = {
  ...
  email: { label: "Business email", helper: "Your protected result is sent after verification.", placeholder: "jane@business.com" },
...
export function GuidedFreeCheckFormFocused({ className }: { className?: string }) {
  ...
  const hasStartedScan = useMemo(() => Object.values(values).some((value) => value.trim().length > 0), [values]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !data || data.ok === false) {
        setSubmitState({ kind: "error", message: data?.error || "The scan could not be submitted yet.", details: data?.details || [] });
        return;
      }
      ...
      setSubmitState({ kind: "success", message: data.message, routingHint: data.routingHint || likelyMove.routingHint, handoff });
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_18rem] lg:items-start">
        <div className="system-panel-authority relative overflow-hidden rounded-[1.55rem] p-4 shadow-[0_24px_80px_rgba(8,47,73,0.2)] sm:p-6 md:p-7">
          ...
          <div className="relative z-10">
            ...
            <div className="mt-5">
              <div className="flex items-center justify-between gap-3 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">
                <span>Step {step + 1} of {STEPS.length}</span>
                <span>{progress}% complete</span>
              </div>
              <div className="system-status-bar mt-2 h-2"><span style={{ width: `${progress}%` }} /></div>
              <p className="mt-3 text-xs leading-5 text-slate-500">Only the current step is shown so the form stays focused and fast.</p>
            </div>

            <form className="mt-6" onSubmit={submit} noValidate>
              <div className="grid gap-4 md:grid-cols-2">
                {activeStep.fields.map((field) => <FieldControl key={field} field={field} values={values} errors={errors} onChange={updateValue} />)}
              </div>
        ...
        <aside className="grid gap-3 lg:sticky lg:top-28">
          <div className="system-panel-authority rounded-[1.35rem] p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-cyan-200">First read</p>
            <h3 className="mt-2 text-xl font-semibold tracking-tight text-white">Focused, not noisy.</h3>
            <p className="mt-2 text-sm leading-6 text-slate-300">The form asks for the details that help Cendorq see visibility, trust, clarity, and action friction.</p>
          </div>
          <div className="system-surface rounded-[1.25rem] p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500">Likely next move</p>
            <h3 className="mt-2 text-lg font-semibold text-white">{likelyMove.title}</h3>
            <p className="mt-2 text-sm leading-6 text-slate-300">{likelyMove.copy}</p>
          </div>
          <div className="sr-only">Free Scan form streamlined. No four visible step badges. Current step only. Dedicated dashboard Free Scan result path after verification.</div>
        </aside>
      </div>
    </section>
  );
...
function buildNextMove(score: number, values: FormValues) {
  ...
  if (score >= 86 && /(ongoing|monthly|monitor|maintain|manage|support)/.test(text)) return { routingHint: "command-review" as RoutingHint, title: "Possible Ongoing Control fit", copy: "Recurring pressure may need continued direction after the first result." };
  if (score >= 78 && /(website|booking|calls|leads|trust|confusing|unclear|conversion|choose)/.test(text)) return { routingHint: "infrastructure-review" as RoutingHint, title: "Possible Build Fix fit", copy: "A specific page, message, proof point, or action path may need scoped improvement." };
  if (score >= 60) return { routingHint: "blueprint-candidate" as RoutingHint, title: "Likely Deep Review fit", copy: "There may be enough detail to diagnose what is holding the business back." };
