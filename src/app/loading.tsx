...
const READOUTS = ["Get found", "Be understood", "Prove trust", "Win choice", "Make action clear"] as const;
    ...
    <main data-cendorq-loading="market-command-loading-v2" className="min-h-screen bg-white px-5 py-16 text-slate-950 sm:px-8">
      <section className="mx-auto flex min-h-[calc(100vh-8rem)] max-w-5xl flex-col items-center justify-center text-center">
        ...
        <p className="mt-7 max-w-2xl text-lg leading-8 text-slate-600">
          Cendorq is loading the next AI readiness path. Start with the Free Scan if the page takes longer than expected.
        </p>

        <div className="mt-10 w-full max-w-2xl rounded-[2rem] border border-slate-200 bg-white p-2 shadow-[0_24px_80px_rgba(15,23,42,0.09)]">
          <div className="grid gap-2 sm:grid-cols-[1fr_auto] sm:items-center">
            <div className="px-5 py-4 text-left">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Safest first read</p>
              <p className="mt-1 text-base font-semibold text-slate-950">See what AI engines and customers may be reading wrong.</p>
            </div>
            <Link href="/free-check" className="inline-flex min-h-14 items-center justify-center rounded-[1.35rem] border border-slate-950 bg-white px-8 py-4 text-base font-semibold text-slate-950 transition hover:bg-slate-950 hover:text-white focus:outline-none focus:ring-2 focus:ring-slate-950 focus:ring-offset-2">
              Start Free Scan
            </Link>
          </div>
        </div>
