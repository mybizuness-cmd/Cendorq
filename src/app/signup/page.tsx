  ...
  return (
    <main className="relative mx-auto max-w-7xl overflow-hidden px-4 py-8 text-white sm:px-6 md:py-10">
      ...
      <section className="relative z-10 grid gap-7 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        <div>
          ...
          <p className="mt-6 max-w-3xl text-base leading-8 text-slate-300 sm:text-lg">
            Your account keeps the first read, reports, billing, support, email follow-up, and plan guidance connected so Cendorq can move you from free insight to the right depth without losing context.
          </p>
        ...
        <div className="system-panel-authority relative overflow-hidden rounded-[1.8rem] p-5 sm:p-7">
          ...
          <h2 className="mt-3 text-3xl font-semibold tracking-tight text-white">
            Verify once. Return easily.
          </h2>
          <p className="mt-3 text-sm leading-7 text-slate-300">
            Choose a provider or create credentials directly. Email confirmation is required before dashboard and result access, and returning customers can use magic-link re-entry.
          </p>
      ...
      <section className="sr-only" aria-label="Signup guardrails">
        Email confirmation before dashboard and result access. Provider sign up or email and password. Email magic link. Magic-link-first re-entry. One-time Cendorq welcome email after verified account creation. Free Scan handoff into your private dashboard. Signup onboarding operating snapshot. Account protection before access. Signup trust rules. Transactional emails and marketing follow-up remain separated. {ONBOARDING_OPERATING_SNAPSHOT.map((item) => `${item.label} ${item.value} ${item.detail}`).join(" ")} {ACCOUNT_PROTECTION_RULES.join(" ")} {SIGNUP_TRUST_RULES.join(" ")} {CUSTOMER_AUTH_METHODS.map((method) => `${method.label} ${method.customerPromise}`).join(" ")} {CUSTOMER_EMAIL_ORCHESTRATION_STEPS.map((step) => `${step.label} ${step.customerPromise}`).join(" ")} {CUSTOMER_EMAIL_DELIVERABILITY_STANDARD.join(" ")} {CUSTOMER_PLATFORM_STAGES.slice(0, 4).map((stage) => `${stage.key} ${stage.label} ${stage.customerPromise}`).join(" ")}
      </section>
