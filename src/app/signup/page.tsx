...
const STEPS = [
  {
    title: "Use email first",
    copy: "Email is the working access path while provider sign-in is connected and tested.",
  },
  {
    title: "Open the inbox",
    copy: "Use the mailbox shortcuts after the secure link is sent. Check spam once if needed.",
  },
  {
    title: "Continue the workspace",
    copy: "The secure link should return to the protected dashboard path that requested access.",
  },
] as const;
  ...
  return (
    <main className={CENDORQ_EXPERIENCE_SYSTEM.pageShell}>
      <section className="relative overflow-hidden px-5 py-8 sm:px-8 lg:py-10">
        ...
        <div className="relative mx-auto grid min-h-[auto] max-w-7xl gap-7 lg:min-h-[min(38rem,calc(100vh-4.25rem))] lg:grid-cols-[0.78fr_1.22fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-5 max-w-5xl text-[clamp(2.7rem,5vw,5.25rem)] font-semibold leading-[0.94] tracking-[-0.076em] text-slate-950">Create account access without breaking the scan path.</h1>
      ...
      <section className="relative mx-auto grid max-w-7xl gap-3 px-5 pb-10 sm:px-8 md:grid-cols-3">
        {STEPS.map((step) => (
          <article key={step.title} className="rounded-[1.45rem] border border-slate-200 bg-white p-4 shadow-[0_12px_38px_rgba(15,23,42,0.045)]">
      ...
      <section className="sr-only" aria-label="Signup access guardrails">
        Sign up. Account access. Create account access without breaking the scan path. Send secure access link. Continue with Google. Continue with Microsoft. Continue with Apple. Continue with LinkedIn. Continue with Facebook. Provider sign-in confirms account identity; Free Scan remains the business-context intake. Secure email link. Disabled unavailable provider buttons. returnTo {returnTo}. {MAIL_PROVIDER_GUARDRAIL_COPY}
      </section>
    </main>
  );
}
