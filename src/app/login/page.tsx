...
export default async function LoginPage({ searchParams }: { searchParams?: LoginSearchParams | Promise<LoginSearchParams> }) {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-white text-slate-950">
      <section className="mx-auto grid min-h-[calc(100vh-4.25rem)] max-w-7xl gap-8 px-5 py-10 sm:px-8 lg:grid-cols-[0.82fr_1.18fr] lg:items-center lg:py-14">
        <div>
          <p className="text-sm font-semibold text-slate-400">Account access</p>
          <h1 className="mt-4 max-w-5xl text-[clamp(2.75rem,5.6vw,5.85rem)] font-semibold leading-[0.95] tracking-[-0.072em] text-slate-950">Return to your Cendorq workspace.</h1>
          <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-600 sm:text-xl sm:leading-9">Use the same email from the Free Scan or payment. If this device is already trusted, Cendorq can continue without making you sign in again.</p>
        ...
        <div className="mx-auto w-full max-w-xl rounded-[2.35rem] border border-slate-200 bg-white p-6 shadow-[0_30px_110px_rgba(15,23,42,0.1)] sm:p-8">
          ...
          <div className="mt-6 rounded-[1.35rem] border border-slate-200 bg-slate-50 p-4 text-center">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">Start with the Free Scan so your workspace is created from business facts, not only account identity.</p>
          ...
          <p className="mt-5 text-center text-xs leading-5 text-slate-500">Cendorq never emails a password. Access uses a secure email link, a trusted session, or a connected provider.</p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-16 sm:px-8" aria-label="Customer access help">
        <div className="rounded-[2.25rem] border border-slate-200 bg-slate-50 p-6 sm:p-8">
          <p className="text-sm font-semibold text-slate-400">How access works</p>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{CUSTOMER_ACCESS_POINTS.map((rule) => <p key={rule} className="rounded-[1.35rem] border border-slate-200 bg-white p-4 text-sm font-semibold leading-7 text-slate-600">{rule}</p>)}</div>
