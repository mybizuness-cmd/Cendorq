...
const RECOVERY_STATES = [
  ["PDF failed", "Recovery", "Retry rendering from the approved package instead of editing by hand."],
  ["Email failed", "Recovery", "Keep the dashboard artifact ready and show a customer-safe delivery status."],
  ["Vault unavailable", "Access", "Route to support until customer access is restored."],
  ["Report held", "Held", "Wait until release approval, entitlement, and PDF readiness align."],
] as const;

const GUARDRAILS = [
  "Show paid reports only when access is confirmed.",
  "Do not send draft, pending, held, or unavailable reports as final.",
  "Keep PDF, email, and dashboard versions aligned.",
  "Keep internal notes, source records, and unapproved links out of customer view.",
] as const;
  ...
  return (
    <main className="relative isolate min-h-screen overflow-hidden bg-[radial-gradient(circle_at_10%_0%,rgba(125,211,252,.28),transparent_30%),radial-gradient(circle_at_86%_0%,rgba(196,181,253,.2),transparent_34%),linear-gradient(180deg,#ffffff_0%,#eefbff_42%,#ffffff_100%)] px-4 py-14 text-slate-950 sm:px-6">
      ...
      <section className="mx-auto grid max-w-[94rem] gap-6 lg:grid-cols-[0.82fr_1.18fr] lg:items-stretch">
        <div className="rounded-[2.25rem] border border-white/80 bg-white/84 p-6 shadow-[0_24px_80px_rgba(15,23,42,.065)] backdrop-blur sm:p-8">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            ...
            <Link href="/dashboard/support" className="rounded-full border border-cyan-200 bg-white px-5 py-3 text-center text-sm font-bold text-cyan-700">Ask delivery support</Link>
          </div>
        </div>
        <section className="rounded-[2.25rem] border border-slate-950 bg-slate-950 p-6 text-white shadow-[0_28px_90px_rgba(15,23,42,.25)] sm:p-8">
          ...
          <h2 className="mt-3 text-4xl font-semibold tracking-[-.065em] text-white sm:text-5xl">Approved package to dashboard to PDF to email to vault record.</h2>
          <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {DELIVERY_EVENTS.map(([label, copy], index) => <article key={label} className="rounded-[1.35rem] border border-white/10 bg-white/[.06] p-4"><span className="grid h-8 w-8 place-items-center rounded-xl bg-cyan-300 text-xs font-black text-slate-950">{index + 1}</span><h3 className="mt-3 text-xl font-semibold tracking-[-.04em] text-white">{label}</h3><p className="mt-2 text-xs font-semibold leading-5 text-slate-300">{copy}</p></article>)}
          </div>
        </section>
      </section>
      ...
      <section className="mx-auto mt-6 max-w-[94rem] rounded-[2rem] border border-white/80 bg-white/86 p-5 shadow-[0_18px_60px_rgba(15,23,42,.055)] backdrop-blur sm:p-6">
        <p className="text-xs font-black uppercase tracking-[0.2em] text-cyan-700">Delivery guardrails</p>
        <div className="mt-5 grid gap-3 md:grid-cols-4">{GUARDRAILS.map((guard) => <p key={guard} className="rounded-[1.15rem] border border-cyan-100 bg-cyan-50/45 p-4 text-xs font-semibold leading-6 text-slate-700">{guard}</p>)}</div>
      </section>
      <section className="sr-only" aria-label="Delivery history guardrails">Delivery History. Dashboard published. PDF prepared. Email sent. PDF downloaded. Resend requested. Delivery failed. Approved package. Dashboard copy. PDF readiness. Email delivery. Report vault. Delivery record. Customer-safe access boundaries.</section>
    </main>
  );
}
