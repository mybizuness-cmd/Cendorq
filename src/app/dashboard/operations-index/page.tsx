...
export const metadata = buildMetadata({
  ...
  description: "Protected Cendorq Operations Index linking reports, delivery, email, attachments, support, plans, business truth, evidence, lineage, visuals, command queue, quality, agents, conversion, entitlements, integrations, analytics, and recovery.",
...
const INDEX_RULES = [
  ...
  "Reports, delivery, email, attachments, support, plans, evidence, lineage, visuals, quality, agents, conversion, and analytics should stay connected rather than becoming separate silos.",
  "The customer dashboard should remain clear even as the operating system grows deeper.",
] as const;

const SYSTEM_GROUPS = [
  ["Customer value", "Reports, delivery, email, attachments, support, conversion, and next commands."],
    ...
    <main className="relative isolate min-h-screen overflow-hidden bg-[radial-gradient(circle_at_10%_0%,rgba(125,211,252,.28),transparent_30%),radial-gradient(circle_at_86%_0%,rgba(196,181,253,.2),transparent_34%),linear-gradient(180deg,#ffffff_0%,#eefbff_42%,#ffffff_100%)] px-4 py-14 text-slate-950 sm:px-6">
      ...
      <section className="mx-auto grid max-w-[94rem] gap-6 lg:grid-cols-[0.82fr_1.18fr] lg:items-stretch">
        <div className="rounded-[2.25rem] border border-white/80 bg-white/84 p-6 shadow-[0_24px_80px_rgba(15,23,42,.065)] backdrop-blur sm:p-8">
          ...
          <p className="mt-6 text-base font-semibold leading-8 text-slate-600">The Operations Index connects every protected surface added to the dashboard so reports, delivery, email, attachments, support, plans, proof, workflows, analytics, conversion, agents, and recovery remain navigable and governed.</p>
      ...
      <section className="sr-only" aria-label="Operations index guardrails">Operations Index. Report vault. Delivery history. Email command. Attachment command. Release gate. Quality gate. Plan matrix. Agent doctrine. Conversion command. Repair workroom. Control center. Support center. Business Truth. Evidence catalog. Report lineage. Visual registry. Command queue. Customer Command Profile. Entitlement center. Integration registry. Analytics center. Audit recovery. Protected customer operating system.</section>
    </main>
  );
}
