...
export default function SupportRequestPage() {
  return (
    <main className="relative isolate min-h-screen overflow-hidden bg-[radial-gradient(circle_at_10%_0%,rgba(251,207,232,0.2),transparent_30%),radial-gradient(circle_at_88%_0%,rgba(125,211,252,0.28),transparent_34%),linear-gradient(180deg,#ffffff_0%,#eefbff_38%,#ffffff_100%)] text-slate-950">
      ...
      <section className="relative mx-auto grid max-w-[92rem] gap-8 px-4 pb-10 pt-8 sm:px-6 md:pt-12 lg:grid-cols-[0.82fr_1.18fr] lg:items-center" aria-label="Support request entry">
        <div className="relative z-10">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-cyan-700">Protected support intake</p>
          <h1 className="max-w-5xl text-[clamp(3rem,7vw,6.6rem)] font-semibold leading-[0.86] tracking-[-0.08em] text-slate-950">Send the safe summary that moves the blocker forward.</h1>
          <p className="mt-6 max-w-3xl text-base font-semibold leading-8 text-slate-600 sm:text-xl sm:leading-9">Intake should collect enough context to help without turning into a private data dump, duplicate request loop, or command-depth shortcut. Work starts only when the right Cendorq gate is clear.</p>
      ...
      <section id="work-start-gates" className="relative mx-auto max-w-[92rem] scroll-mt-8 px-4 pb-8 sm:px-6" aria-label="Cendorq work start gates">
        <div className="overflow-hidden rounded-[2.15rem] border border-white/80 bg-white/84 shadow-[0_18px_60px_rgba(15,23,42,0.055)] backdrop-blur">
          <div className="border-b border-cyan-100 p-5 sm:p-7">
          ...
          <div className="grid gap-0 lg:grid-cols-3">
            {CENDORQ_WORK_START_GATE_PROJECTIONS.map((gate) => (
              <article key={gate.key} className="border-b border-cyan-100 p-5 lg:border-b-0 lg:border-r last:lg:border-r-0">
                <div className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-700">{gate.customerTitle}</div>
                ...
                <div className="mt-4 rounded-[1rem] border border-cyan-100 bg-cyan-50/45 p-3"><div className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-700">Backend rule</div><p className="mt-2 text-xs font-semibold leading-6 text-slate-600">{gate.backendStartRule}</p></div>
                <div className="mt-3 rounded-[1rem] border border-cyan-100 bg-white p-3"><div className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-700">Required before queue</div><ul className="mt-2 grid gap-2 text-xs font-semibold leading-5 text-slate-600">{gate.requiredBeforeQueue.slice(0, 4).map((item) => <li key={item}>• {item}</li>)}</ul></div>
