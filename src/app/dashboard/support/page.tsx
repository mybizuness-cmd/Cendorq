...
export default function SupportCenterPage() {
  return (
    <main className="relative isolate min-h-screen overflow-hidden bg-[radial-gradient(circle_at_10%_0%,rgba(251,207,232,0.2),transparent_30%),radial-gradient(circle_at_88%_0%,rgba(125,211,252,0.28),transparent_34%),linear-gradient(180deg,#ffffff_0%,#eefbff_38%,#ffffff_100%)] text-slate-950">
      ...
      <section className="relative mx-auto grid max-w-[92rem] gap-8 px-4 pb-10 pt-8 sm:px-6 md:pt-12 lg:grid-cols-[0.82fr_1.18fr] lg:items-center" aria-label="Support routing entry">
        <div className="relative z-10">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-cyan-700">AI Visibility support routing</p>
          <h1 className="max-w-5xl text-[clamp(3rem,7vw,6.6rem)] font-semibold leading-[0.86] tracking-[-0.08em] text-slate-950">Route the blocker without weakening the command path.</h1>
          <p className="mt-6 max-w-3xl text-base font-semibold leading-8 text-slate-600 sm:text-xl sm:leading-9">Help should restore momentum, protect the proof trail, and return the customer to the right report, account, AI Visibility depth, or status path.</p>
        ...
        <div className="relative overflow-hidden rounded-[2.35rem] border border-white/80 bg-white/78 p-5 shadow-[0_30px_100px_rgba(15,23,42,0.09)] backdrop-blur-2xl sm:p-7">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {SUPPORT_ROUTES.slice(0, 2).map((route) => (
              <Link key={route.key} href={route.href} className="rounded-[1.3rem] border border-cyan-100 bg-cyan-50/42 p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-cyan-200 hover:bg-white focus:outline-none focus:ring-2 focus:ring-cyan-300 focus:ring-offset-2">
                <div className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-700">{route.label}</div>
          ...
          <div className="grid gap-0 lg:grid-cols-[0.72fr_1.28fr]">
            ...
            <div className="grid gap-0 md:grid-cols-2">
              {SUPPORT_ROUTES.map((route) => (
                <Link key={route.key} href={route.href} className="group border-b border-cyan-100 p-5 transition hover:-translate-y-0.5 hover:bg-cyan-50 focus:outline-none focus:ring-2 focus:ring-cyan-300 focus:ring-offset-2 md:border-r">
                  ...
                  <p className="mt-3 rounded-[1rem] border border-cyan-100 bg-cyan-50/45 p-3 text-xs font-semibold leading-5 text-slate-600">{route.boundary}</p>
      ...
      <section className="relative mx-auto max-w-[92rem] px-4 pb-8 sm:px-6" aria-label="Support triage consistency">
        <div className="rounded-[1.75rem] border border-cyan-100 bg-cyan-50/42 p-5 shadow-[0_14px_42px_rgba(15,23,42,0.04)] sm:p-6">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-cyan-700">Support triage</p>
      ...
      <section className="relative mx-auto max-w-[92rem] px-4 pb-8 sm:px-6" aria-label="AI Visibility support boundaries">
        <div className="grid gap-3 lg:grid-cols-4">
          {PLAN_SUPPORT.map((plan) => (
            <Link key={plan.planKey} href={plan.href} className="rounded-[1.55rem] border border-white/80 bg-white/82 p-5 shadow-[0_14px_42px_rgba(15,23,42,0.045)] backdrop-blur transition hover:-translate-y-0.5 hover:border-cyan-200 hover:bg-cyan-50 focus:outline-none focus:ring-2 focus:ring-cyan-300 focus:ring-offset-2">
              ...
              <p className="mt-2 text-[10px] font-bold uppercase tracking-[0.16em] text-cyan-700">{plan.value.customerName}</p>
              <p className="mt-3 text-xs font-semibold leading-6 text-slate-600">{plan.role}</p>
              <p className="mt-3 rounded-[1rem] border border-cyan-100 bg-cyan-50/45 p-3 text-xs font-semibold leading-5 text-slate-600">{plan.mustNot}</p>
