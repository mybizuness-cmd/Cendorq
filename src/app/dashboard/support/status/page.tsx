    ...
    <main className="relative mx-auto max-w-7xl overflow-hidden px-4 py-5 text-white sm:px-6 md:py-10">
      ...
      <section className="system-panel-authority relative z-10 overflow-hidden rounded-[1.8rem] p-4 shadow-[0_34px_130px_rgba(2,8,23,0.52)] sm:p-8">
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_20rem] lg:items-start">
          <div>
            <p className="text-sm font-semibold text-cyan-100">Market resolution status</p>
            ...
            <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300 sm:mt-5 sm:text-base sm:leading-8">
              Status should reduce anxiety, protect private information, and return the customer to proof, billing, support, or command depth as soon as the blocker is clear.
            </p>
          </div>
          <div className="rounded-[1.25rem] border border-cyan-300/20 bg-cyan-300/10 p-4 sm:p-5">
            ...
            <h2 className="mt-2 text-2xl font-semibold tracking-tight text-white">Return to the right command.</h2>
            ...
            <Link href="/dashboard/billing" className="mt-4 inline-flex min-h-11 w-full items-center justify-center rounded-2xl bg-cyan-300 px-5 py-3 text-sm font-bold text-slate-950 transition hover:bg-cyan-200 focus:outline-none focus:ring-2 focus:ring-cyan-100 focus:ring-offset-2 focus:ring-offset-slate-950">Open command depth</Link>
          </div>
        </div>
      </section>
      ...
      <section className="relative z-10 mt-7 rounded-[1.55rem] border border-white/10 bg-white/[0.035] p-4 sm:p-5" aria-label="Support status safety standard">
        <p className="text-sm font-semibold text-cyan-100">Status safety standard</p>
        <h2 className="mt-2 text-2xl font-semibold tracking-tight text-white sm:text-4xl">Show progress without exposing internals.</h2>
