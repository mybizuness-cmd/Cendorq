...
const DASHBOARD_DECISION = [
  { label: "Business state", value: "First signal needed", detail: "The dashboard starts by proving where the customer decision is breaking." },
  ...
  { label: "Not unlocked yet", value: "Paid depth", detail: "Deep Review, Build Fix, and Ongoing Control stay separate until the stage fits." },
] as const;
    ...
    <main className="relative mx-auto max-w-7xl overflow-hidden px-4 py-5 text-white sm:px-6 md:py-10 xl:py-12">
      ...
      <section className="system-panel-authority relative z-10 overflow-hidden rounded-[1.55rem] p-4 shadow-[0_28px_110px_rgba(2,8,23,0.42)] sm:rounded-[1.8rem] sm:p-8">
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_20rem] lg:items-start">
          <div>
            <p className="text-sm font-semibold text-cyan-100">Private business command center</p>
            ...
            <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300 sm:mt-5 sm:text-base sm:leading-8">
              This dashboard is not an account page. It is the operating surface for the next business decision: finish the first signal, read the proof, and choose the right depth only when it fits.
            </p>
          </div>
          <div className="rounded-[1.25rem] border border-cyan-300/20 bg-cyan-300/10 p-4 sm:p-5">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-200">Then use the result to decide whether Deep Review, Build Fix, or Ongoing Control actually fits.</p>
      ...
      <section className="relative z-10 mt-7 overflow-hidden rounded-[1.7rem] border border-cyan-300/15 bg-[linear-gradient(135deg,rgba(8,47,73,0.72),rgba(2,8,23,0.88)_48%,rgba(14,116,144,0.24))] p-4 shadow-[0_28px_100px_rgba(2,8,23,0.42)] sm:p-7" aria-label="Dashboard premium plan path">
        ...
        <div className="mt-5 grid gap-3 xl:grid-cols-4">
          {CUSTOMER_REVENUE_COMMAND_PATH.map((stage) => (
            <Link key={stage.planKey} href={stage.href} className="group relative overflow-hidden rounded-[1.35rem] border border-white/10 bg-slate-950/60 p-5 transition hover:-translate-y-0.5 hover:border-cyan-300/35 hover:bg-slate-900/80 focus:outline-none focus:ring-2 focus:ring-cyan-200 focus:ring-offset-2 focus:ring-offset-slate-950">
              <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-cyan-200/50 to-transparent" />
              <div className="flex items-start justify-between gap-3">
                <span className="text-3xl font-semibold tracking-tight text-cyan-100/80">{stage.index}</span>
              ...
              <p className="mt-5 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">{stage.stage}</p>
              <h3 className="mt-2 text-2xl font-semibold tracking-tight text-white">{stage.value.customerName}</h3>
              <p className="mt-3 text-sm leading-6 text-slate-300">{stage.buyerMoment}</p>
