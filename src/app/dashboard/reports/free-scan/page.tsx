...
export const metadata = buildMetadata({
  title: "Readiness signal result | Cendorq",
  description: "Your protected Cendorq readiness signal result, AI-readiness posture, confidence limits, and next action.",
...
const RESULT_STATE = [
  { label: "Layer", value: "Scan", detail: "The first readiness signal before paid review or implementation." },
  { label: "AI-readiness posture", value: "First signal only", detail: "The result can show readiness risk without claiming ranking, placement, or full review." },
  { label: "Next layer", value: "Review", detail: "Use AI Readiness Review only when the signal matters enough to prove the cause." },
] as const;
...
const RESULT_DECISION = [
  ...
  { title: "Next layer", copy: "The cleanest next move when the first signal matters enough to review, repair, or control over time." },
] as const;
...
export default function FreeScanResultsPage() {
  return (
    <main className="relative isolate overflow-hidden text-white">
      ...
      <section className="relative mx-auto grid min-h-[calc(100vh-5rem)] max-w-[92rem] gap-8 px-4 pb-12 pt-6 sm:px-6 md:pb-18 md:pt-10 lg:grid-cols-[0.88fr_1.12fr] lg:items-center">
        <div className="relative z-10">
          ...
          <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-300 sm:text-xl sm:leading-9">
            This is the Scan layer inside the customer dashboard. It shows what may be visible, unclear, weak, or blocked before Cendorq claims a full review.
          </p>
        ...
        <div className="relative overflow-hidden rounded-[2.7rem] border border-cyan-200/22 bg-[radial-gradient(circle_at_50%_0%,rgba(103,232,249,0.2),transparent_36%),linear-gradient(145deg,rgba(8,47,73,0.9),rgba(2,8,23,0.98)_52%,rgba(14,116,144,0.32))] p-5 shadow-[0_55px_200px_rgba(2,8,23,0.72)] sm:p-7">
          ...
          <p className="text-sm font-black uppercase tracking-[0.2em] text-cyan-100">Next layer</p>
      ...
      <section className="relative mx-auto max-w-[92rem] px-4 pb-10 sm:px-6" aria-label="Readiness signal state">
      ...
      <section className="relative mx-auto max-w-[92rem] px-4 pb-10 sm:px-6" aria-label="Readiness signal intelligence">
        <div className="overflow-hidden rounded-[2.5rem] border border-cyan-300/15 bg-[linear-gradient(135deg,rgba(8,47,73,0.72),rgba(2,8,23,0.94)_46%,rgba(14,116,144,0.22))] shadow-[0_45px_180px_rgba(2,8,23,0.55)]">
          <div className="grid gap-0 lg:grid-cols-[0.74fr_1.26fr]">
            <div className="border-b border-white/10 p-6 sm:p-8 lg:border-b-0 lg:border-r lg:p-10">
              ...
              <Link href="/dashboard/reports" className="mt-7 inline-flex text-sm font-bold text-cyan-100 transition hover:text-white focus:outline-none focus:ring-2 focus:ring-cyan-200 focus:ring-offset-2 focus:ring-offset-slate-950">Back to proof vault →</Link>
            </div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {SAMPLE_FINDINGS.slice(0, 3).map((finding) => (
            <article key={`${finding.axis}-${finding.findingLabel}`} className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-6 shadow-[0_24px_90px_rgba(2,8,23,0.34)]">
              ...
              <p className="mt-4 rounded-[1.2rem] border border-cyan-300/15 bg-cyan-300/[0.08] p-4 text-sm leading-7 text-cyan-50">AI-readiness: {finding.aiVisibilityImpact}</p>
      ...
      <section className="relative mx-auto grid max-w-[92rem] gap-4 px-4 pb-10 sm:px-6 lg:grid-cols-2" aria-label="Readiness signal boundaries">
      ...
      <section className="relative mx-auto max-w-[92rem] px-4 pb-16 sm:px-6" aria-label="Free Scan methodology summary">
        <div className="overflow-hidden rounded-[2.5rem] border border-white/10 bg-[linear-gradient(135deg,rgba(255,255,255,0.07),rgba(255,255,255,0.025)_38%,rgba(103,232,249,0.08))] p-6 shadow-[0_45px_180px_rgba(2,8,23,0.55)] backdrop-blur-2xl sm:p-8 lg:p-10">
          ...
          <h2 className="mt-3 max-w-5xl text-4xl font-semibold tracking-[-0.055em] text-white sm:text-6xl">Evidence, confidence, AI-readiness posture, and priority stay separate.</h2>
      ...
      <section className="sr-only" aria-label="Free Scan result guardrails">
        Readiness signal result. Protected dashboard result. Dashboard-only Free Scan result route. Free Scan result page must remain under /dashboard/reports/free-scan and not public. The first AI-readiness signal is ready. Signal Proof Risk Limit Next layer. Scan layer. Bounded confidence. Signal intelligence. Useful because it explains the limit before the next move. Evidence confidence AI-readiness posture and priority stay separate. What this signal includes. What stays outside Scan. Free Scan does not include full root-cause review, implementation work, monthly monitoring, guaranteed ranking, or guaranteed AI placement. AI Readiness Review finds the full reason. No overlap. {FREE_SCAN_AI_VISIBILITY_MODEL.promise} {FREE_SCAN_AI_VISIBILITY_MODEL.customerTruth} {FREE_SCAN_AI_VISIBILITY_MODEL.reportRule} {FREE_SCAN_VALUE.reportBoundary} {FREE_SCAN_VALUE.upgradeLogic} {PLAN_VALUE_SEPARATION_RULES.join(" ")} Six signals. Clarity Trust Choice Action AI/Search Visibility Proof. Observed evidence. Inferred judgment. Needs deeper review. Evidence rules. Priority model. Critical Important Watch. AI Readiness Review $497. Accurate results require confidence posture and limitations. {FREE_SCAN_REPORT_QUALITY_RULES.join(" ")}
      </section>
    </main>
  );
}
