      ...
      <section className="relative overflow-hidden px-5 pb-8 pt-7 sm:px-8 lg:min-h-[min(39rem,calc(100vh-4.25rem))] lg:pb-10 lg:pt-8 xl:min-h-[min(42rem,calc(100vh-4.25rem))]">
        ...
        <div className="relative mx-auto grid w-full max-w-7xl gap-6 lg:grid-cols-[minmax(0,0.94fr)_minmax(24rem,1.06fr)] lg:items-center">
          <div className="max-w-[43rem] text-left">
            <h1 className="text-[clamp(2.55rem,11vw,5rem)] font-semibold leading-[0.91] tracking-[-0.08em] text-slate-950 lg:text-[clamp(3rem,4.85vw,5.35rem)]">If AI engines cannot understand your business, customers may never get the chance to.</h1>
            <p className="mt-4 max-w-2xl text-base font-medium leading-7 text-slate-600 sm:text-lg sm:leading-8">Cendorq turns AI-readiness into a clear path: see the first weak signal, understand what is causing it, improve what matters, and keep readiness from drifting.</p>
    ...
    <div className="relative mx-auto w-full max-w-[34rem] lg:ml-auto" aria-hidden="true">
      ...
      <div className="relative overflow-hidden rounded-[2rem] border border-white/90 bg-white/62 p-3 shadow-[0_24px_80px_rgba(15,23,42,0.11)] backdrop-blur-2xl sm:rounded-[2.7rem]">
        <div className="relative min-h-[29rem] overflow-hidden rounded-[1.55rem] border border-slate-200 bg-[radial-gradient(circle_at_48%_30%,#ffffff_0%,#effcff_42%,#dff4ff_100%)] p-5 shadow-inner sm:min-h-[27rem] sm:rounded-[2.2rem] sm:p-7">
          ...
          <div className="relative mx-auto mt-4 h-[18.5rem] w-full max-w-[21rem] sm:h-60 sm:max-w-none">
            <div className="absolute left-1/2 top-1/2 z-20 flex h-[6.6rem] w-[6.6rem] -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-slate-900 bg-slate-950 text-white shadow-[0_18px_56px_rgba(15,23,42,0.25)] sm:h-[7rem] sm:w-[7rem]"><span className="text-lg font-semibold tracking-[-0.04em] text-white sm:text-xl">Cendorq</span></div>
            <div className="absolute left-[21%] top-[25%] h-[2px] w-[30%] rotate-[20deg] bg-cyan-400/90" /><div className="absolute right-[21%] top-[25%] h-[2px] w-[30%] -rotate-[20deg] bg-cyan-400/90" /><div className="absolute bottom-[25%] left-[21%] h-[2px] w-[30%] -rotate-[20deg] bg-cyan-400/90" /><div className="absolute bottom-[25%] right-[21%] h-[2px] w-[30%] rotate-[20deg] bg-cyan-400/90" />
            {READINESS_PATH.map((stage, index) => (
              <div key={stage.label} className={`absolute z-30 ${positions[index]} flex h-[6.7rem] w-[8.55rem] flex-col justify-between rounded-[1.05rem] border border-white/95 bg-white/94 p-3 shadow-[0_12px_30px_rgba(15,23,42,0.09)] backdrop-blur sm:h-[6.8rem] sm:w-[8.8rem]`}>
