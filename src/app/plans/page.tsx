...
const PLAN_CARDS = CENDORQ_PLAN_PRICES.map((plan) => ({
  ...
  accent: PLAN_ACCENT_BY_KEY[plan.key],
}));

const DECISION_RULES = [
  "Unsure? Start Free Scan.",
  "Need the cause? Open Review.",
  "Know what to improve? Open Repair.",
  "Need ongoing watch? Open Control.",
] as const;
    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,#fff7fb_0%,#e9fbff_18%,#eff9ff_62%,#ffffff_100%)] text-slate-950">
      ...
      <section className="relative overflow-hidden px-5 py-8 sm:px-8 lg:py-10">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-7 lg:min-h-[min(38rem,calc(100vh-4.25rem))] lg:grid-cols-[0.68fr_1.32fr] lg:items-center">
          <div>
            <h1 className="max-w-5xl text-[clamp(2.7rem,5vw,5.25rem)] font-semibold leading-[0.94] tracking-[-0.078em] text-slate-950">
            ...
            <p className="mt-4 max-w-3xl text-base font-medium leading-7 text-slate-600 sm:text-lg sm:leading-8">
              Plans is the decision hub. Pick the right depth, open that plan page, then continue straight to secure payment from the plan page.
            </p>
            <div className="mt-5 grid gap-2 text-sm font-semibold text-slate-700">
              {DECISION_RULES.map((rule) => <p key={rule} className="rounded-full border border-white/80 bg-white/78 px-4 py-2 shadow-sm backdrop-blur">{rule}</p>)}
            </div>
                ...
                {PLAN_CARDS.map((plan) => (
                  <article key={plan.key} className={`rounded-[1.45rem] p-4 sm:rounded-[1.65rem] ${plan.accent}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-xs font-bold text-slate-500">{plan.stage}</p>
