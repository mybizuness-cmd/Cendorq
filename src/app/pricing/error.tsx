...
const recoveryRoutes = [
    {
        title: "Retry the plans route",
        copy:
            "Use this first if the interruption looks temporary and you want the pricing route to attempt a clean recovery.",
        type: "retry" as const,
    },
    {
        title: "Return to the Search Presence Scan",
        copy:
            "Use this if you want to re-enter through the strongest first step instead of forcing the plan page while it is unstable.",
        href: "/free-check",
        label: "Run Search Presence Scan",
    },
    {
        title: "Review how the system works",
        copy:
            "Use this if you want to confirm the sequence first, then come back to pricing through a clearer route.",
        href: "/diagnosis",
        label: "See how it works",
    },
];

export default function PricingErrorPage({
    error,
    reset,
}: PricingErrorPageProps) {
    useEffect(() => {
        ...
        <main className="relative mx-auto max-w-7xl overflow-hidden px-4 py-12 text-white sm:px-6 md:py-16 xl:py-20">
            ...
            <section className="relative z-10 max-w-5xl">
                ...
                <p className="mt-6 max-w-4xl text-base leading-8 text-slate-300 sm:text-lg">
                    The pricing page failed while trying to load or render the plan
                    architecture. That does not automatically mean the whole system is
                    down. It means this specific comparison route needs a clean retry or a
                    cleaner re-entry path.
                </p>

                <div className="mt-8 flex flex-wrap gap-3">
                    <AuthorityPill>Retry once</AuthorityPill>
                    <AuthorityPill>Use a clean route</AuthorityPill>
                    <AuthorityPill>Do not guess through instability</AuthorityPill>
                </div>
            </section>
            ...
            <section className="relative z-10 mt-16 grid gap-6 lg:grid-cols-[1.02fr_0.98fr]">
                <div className="system-panel-authority rounded-[2rem] p-6 sm:p-8 md:p-10">
                    ...
                    <h2 className="mt-5 max-w-4xl text-3xl font-semibold tracking-tight text-white sm:text-4xl md:text-5xl">
                        The comparison layer broke before the business could choose the next step cleanly.
                    </h2>

                    <p className="mt-6 max-w-3xl text-base leading-8 text-slate-300">
                        The plans page matters because it helps the business decide between
                        first signal, deeper explanation, one-time improvement, and monthly
                        continuity. When that route fails, the right move is not random
                        clicking. The right move is a controlled recovery.
                    </p>

                    <p className="mt-5 max-w-3xl text-base leading-8 text-slate-300">
                        Retry the route once. If it still fails, step back into a stable path
                        and re-enter through the stronger sequence instead of forcing the
                        broken comparison layer.
                    </p>

                    <div className="mt-8 grid gap-4 sm:grid-cols-3">
                        <StatusTile label="Signal-first route" value="Available" />
                        <StatusTile label="Plan comparison" value="Interrupted" highlighted />
                ...
                <div className="grid gap-4">
                    <ReadoutCard
                        label="Route state"
                        value="Pricing route failure captured"
                        copy="The interruption occurred at the plan-comparison layer, not necessarily across the full system."
                    />
                    <ReadoutCard
                        label="Best next move"
                        value="Retry once, then re-enter cleanly"
                        copy="If the failure is temporary, a reset may recover the route. If not, go back through Search Presence Scan or How It Works."
                    />
                    <ReadoutCard
                        label="Reference"
                        value={reference}
                        copy="Use this internal reference if you need to identify the route failure more precisely later."
                        breakWords
                    />
                </div>
            </section>
                ...
                <div className="system-panel-authority rounded-[2rem] p-6 sm:p-8 md:p-10">
                    <div className="grid gap-8 lg:grid-cols-[1.08fr_0.92fr] lg:items-center">
                        <div>
                            ...
                            <h2 className="mt-5 max-w-4xl text-3xl font-semibold tracking-tight text-white sm:text-4xl md:text-5xl">
                                Do not choose the next step from a broken comparison route.
                            </h2>

                            <p className="mt-6 max-w-3xl text-base leading-8 text-slate-300">
                                The stronger move is to go back into a stable path, recover the
                                system state, and then compare plans through a clean page load.
                                That keeps the experience authoritative instead of chaotic.
                            </p>
                        ...
                        <div className="grid gap-4">
                            ...
                            <InfoTile label="Sequence explanation" value="How It Works" />
                            <InfoTile label="Comparison route" value="Retry once only" />
                        </div>
                    </div>
                </div>
            </section>
...
function PricingErrorAtmosphere() {
    return (
