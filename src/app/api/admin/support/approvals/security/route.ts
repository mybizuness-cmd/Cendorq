...
type SecurityApprovalPayload = Record<string, unknown>;

type SafeSecurityApprovalResponse = {
  ok: true;
...
const MAX_SECURITY_APPROVAL_BYTES = 14_000;
...
const REQUESTER_ROLES = ["support-triage", "support-specialist", "security-reviewer", "support-admin"] as const satisfies readonly CustomerSupportOperatorRole[];
...
export async function POST(request: NextRequest) {
  ...
  if (access.operatorRole !== "security-reviewer") return jsonNoStore({ ok: false, error: "Security approval requires a security reviewer session.", details: ["Use the protected security approval path with the correct reviewer role."] }, 403);

  const contentLength = Number(request.headers.get("content-length") || "0");
  if (Number.isFinite(contentLength) && contentLength > MAX_SECURITY_APPROVAL_BYTES) return jsonNoStore({ ok: false, error: "The security approval request is too large to process safely.", details: ["Submit a shorter customer-safe security summary."] }, 413);

  let rawBody = "";
  try {
    rawBody = await request.text();
  } catch {
    return jsonNoStore({ ok: false, error: "The security approval body could not be read.", details: ["Submit a valid JSON security approval request."] }, 400);
  }
  if (Buffer.byteLength(rawBody, "utf8") > MAX_SECURITY_APPROVAL_BYTES) return jsonNoStore({ ok: false, error: "The security approval request is too large to process safely.", details: ["Submit a shorter customer-safe security summary."] }, 413);

  let payload: SecurityApprovalPayload;
  try {
    const parsed = JSON.parse(rawBody) as unknown;
    if (!isRecord(parsed)) throw new Error("payload must be object");
    payload = parsed;
  } catch {
    return jsonNoStore({ ok: false, error: "The security approval payload is not valid JSON.", details: ["Submit a valid JSON security approval request."] }, 400);
  }
  ...
  if (!decision) fieldErrors.decision = "Allowed security approval decision is required.";
  if (!state) fieldErrors.state = "Allowed security approval state is required.";
  if (!reasonCode) fieldErrors.reasonCode = "Safe reason code is required.";
  if (customerSafeSummary.length < 20) fieldErrors.customerSafeSummary = "Customer-safe security approval summary of at least 20 characters is required.";
  if (decision === "approve" && customerSafeOutcomeCopy.length < 20) fieldErrors.customerSafeOutcomeCopy = "Customer-safe security outcome copy is required before approval.";
  if (Object.keys(fieldErrors).length) return jsonNoStore({ ok: false, error: "The security approval request needs required safe fields.", fieldErrors }, 400);
  if (!requestedByRole || !decision || !state) return jsonNoStore({ ok: false, error: "The security approval request could not be normalized safely.", details: ["Use allowlisted security approval values only."] }, 400);
  ...
  try {
    ...
    if (!auditBuild.ok) return jsonNoStore({ ok: false, error: "The security approval audit record could not be created safely.", details: [auditBuild.reason] }, 400);
    ...
    if (!approvalBuild.ok) return jsonNoStore({ ok: false, error: "The security approval record could not be created safely.", details: [approvalBuild.reason] }, 400);
    ...
    const body: SafeSecurityApprovalResponse = {
      ok: true,
    ...
    return jsonNoStore({ ok: false, error: "The security approval could not be stored cleanly.", details: ["The security approval storage layer could not complete the safe mutation."] }, 500);
  }
}
