  ...
  return (
    <main className="relative mx-auto max-w-6xl overflow-hidden px-4 pb-28 pt-8 text-white sm:px-6 md:py-14">
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_18rem] lg:items-start">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300 sm:mt-5 sm:text-base sm:leading-8">
              Every paid plan keeps the customer account, business context, report history, and next step connected so work can start immediately after payment.
            </p>
          </div>
          <div className="rounded-[1.2rem] border border-cyan-300/20 bg-cyan-300/10 p-4 sm:rounded-[1.3rem] sm:p-5">
            <div className="text-sm font-semibold text-cyan-100">Default next path</div>
      ...
      <section id="stripe-link-needed" className="relative z-10 mt-7 grid gap-4 md:grid-cols-3" aria-label="Checkout preparation">
        <CheckoutPrepCard title="1. Correct plan" copy="Each paid plan will use the exact Stripe link or price ID you provide." />
        ...
        <CheckoutPrepCard title="3. Work starts after payment" copy="Payment confirmation should create the entitlement, billing record, dashboard notification, work queue, and email handoff." />
      </section>

      <section className="relative z-10 mt-7 rounded-[1.35rem] border border-white/10 bg-white/[0.035] p-5">
        <h2 className="text-2xl font-semibold tracking-tight text-white">While the Stripe links are being added</h2>
        <p className="mt-3 text-sm leading-7 text-slate-300">
          This page is ready for the plan-specific Stripe checkout links. Once those links are added, each paid CTA can point directly to the correct checkout path and return to the plan-specific success page.
        </p>
