...
export const metadata = buildMetadata({
  ...
  description: "Start a Cendorq checkout for AI Readiness Review, Signal Repair, or Readiness Control.",
  path: "/checkout/start",
  noIndex: true,
});
...
const CHECKOUT_HANDOFFS = [
  {
    ...
    title: "AI Readiness Review checkout",
    command: "Review",
    next: "Unlock evidence-backed review and the report workflow.",
  },
  {
    plan: BUILD_FIX,
    value: getPlanValueDelivery("build-fix"),
    title: "Signal Repair checkout",
    command: "Repair",
    next: "Unlock scoped implementation after the weak signal is clear.",
  },
  {
    plan: ONGOING_CONTROL,
    value: getPlanValueDelivery("ongoing-control"),
    title: "Readiness Control checkout",
    command: "Control",
        ...
        <div className="overflow-hidden rounded-[2.5rem] border border-cyan-300/15 bg-[linear-gradient(135deg,rgba(8,47,73,0.72),rgba(2,8,23,0.94)_46%,rgba(14,116,144,0.22))] shadow-[0_45px_180px_rgba(2,8,23,0.55)]">
          <div className="grid gap-0 lg:grid-cols-[0.78fr_1.22fr]">
            <div className="border-b border-white/10 p-6 sm:p-8 lg:border-b-0 lg:border-r lg:p-10">
              ...
              <h2 className="mt-4 text-4xl font-semibold tracking-[-0.06em] text-white sm:text-6xl">Payment should unlock the exact readiness layer.</h2>
              <p className="mt-5 text-base leading-8 text-slate-300">Review, Repair, and Control must not collapse into the same payment card. The handoff has to preserve the plan boundary.</p>
            ...
            <div className="divide-y divide-white/10">
              {CHECKOUT_HANDOFFS.map((item) => (
                <Link key={item.plan.key} href={item.plan.checkoutPath} className="group grid gap-4 p-5 transition hover:bg-cyan-200/[0.06] focus:outline-none focus:ring-2 focus:ring-cyan-200 focus:ring-offset-2 focus:ring-offset-slate-950 sm:grid-cols-[11rem_1fr_auto] sm:items-center sm:p-6">
                  <div>
