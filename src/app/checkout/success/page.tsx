...
export const metadata = buildMetadata({
  title: "Readiness activated | Cendorq",
  description: "Your Cendorq readiness layer is unlocked. Activate Review, Repair, or Control from the protected dashboard.",
...
const PLAN_ACTIVATION_COPY: Record<CendorqPaidPlanKey, { command: string; activationLabel: string; dashboardCta: string; dashboardPath: string; unlockedState: string; boundaryReminder: string; customerMoment: string }> = {
  "deep-review": {
    command: "Review",
    activationLabel: "Review activation",
    dashboardCta: "Open report vault",
    dashboardPath: "/dashboard/reports",
    unlockedState: "AI Readiness Review is now active. The next job is to confirm what Cendorq should prove first.",
    boundaryReminder: "AI Readiness Review unlocks evidence-backed review and decision clarity, not done-for-you implementation or recurring monitoring.",
    customerMoment: "You paid to understand the real reason before spending more money.",
  },
  "build-fix": {
    command: "Repair",
    activationLabel: "Repair activation",
    dashboardCta: "Confirm repair scope",
    dashboardPath: "/dashboard/support/request",
    unlockedState: "Signal Repair is now active. The next job is to confirm the scoped target and approved business details.",
    boundaryReminder: "Signal Repair unlocks scoped improvement work, not a full review report, unlimited site rebuild, or monthly control loop.",
    customerMoment: "You paid to improve a known weak signal with bounded implementation work.",
  },
  "ongoing-control": {
    command: "Control",
    ...
    unlockedState: "Readiness Control is now active. The next job is to choose the first monthly priority and monitoring scope.",
    boundaryReminder: "Readiness Control unlocks recurring monitoring and monthly decision support, not unlimited Signal Repair work or guaranteed ranking/AI placement.",
...
const PAID_REPORT_DELIVERY_CONFIRMATION =
  "Paid report delivery confirmation: every paid report must appear in the dashboard report vault first. Email, dashboard message mirror, downloadable PDF, or PDF attachment delivery is enabled only when verification, entitlement, release, and document-safety gates pass.";

const ACTIVATION_STEPS = [
  { label: "Payment", value: "Complete", detail: "The readiness layer can now attach to the protected workspace." },
...
export default async function CheckoutSuccessPage({ searchParams }: CheckoutSuccessPageProps) {
  ...
  return (
    <main className="relative isolate overflow-hidden text-white">
      ...
      <section className="relative mx-auto grid min-h-[calc(100vh-5rem)] max-w-[92rem] gap-8 px-4 pb-12 pt-6 sm:px-6 md:pb-18 md:pt-10 lg:grid-cols-[0.88fr_1.12fr] lg:items-center">
        <div className="relative z-10">
          ...
          <h1 className="mt-6 max-w-5xl text-[clamp(3.2rem,7.3vw,7.5rem)] font-semibold leading-[0.84] tracking-[-0.08em] text-white">
            {activation.command} is unlocked. Activate the readiness path.
          </h1>
          <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-300 sm:text-xl sm:leading-9">
            {activation.customerMoment} Cendorq now has to route the work into the dashboard, preserve the boundary, and collect the context needed for delivery.
          </p>
      ...
      <section className="relative mx-auto max-w-[92rem] px-4 pb-10 sm:px-6" aria-label="Readiness activation after payment">
        <div className="overflow-hidden rounded-[2.5rem] border border-cyan-300/15 bg-[linear-gradient(135deg,rgba(8,47,73,0.72),rgba(2,8,23,0.94)_46%,rgba(14,116,144,0.22))] shadow-[0_45px_180px_rgba(2,8,23,0.55)]">
          <div className="grid gap-0 lg:grid-cols-[0.78fr_1.22fr]">
            <div className="border-b border-white/10 p-6 sm:p-8 lg:border-b-0 lg:border-r lg:p-10">
              ...
              <h2 className="mt-4 text-4xl font-semibold tracking-[-0.06em] text-white sm:text-6xl">Payment is complete. The readiness path starts now.</h2>
      ...
      <section className="relative mx-auto grid max-w-[92rem] gap-4 px-4 pb-10 sm:px-6 lg:grid-cols-2" aria-label="Plan value boundaries after payment">
