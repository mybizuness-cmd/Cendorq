...
const POST_PAYMENT_CONTEXT: Record<CendorqPaidPlanKey, { stage: string; title: string; dashboardCta: string; completedEvidence: readonly ["customerOwnershipVerified"]; completedIntake: readonly string[] }> = {
  "deep-review": {
    ...
    title: "Your AI Readiness Review is confirmed.",
    dashboardCta: "Open review next step",
    completedEvidence: ["customerOwnershipVerified"],
    completedIntake: [],
  },
  "build-fix": {
    stage: "Signal Repair",
    title: "Your Signal Repair payment is confirmed.",
    dashboardCta: "Open repair requirements",
    completedEvidence: ["customerOwnershipVerified"],
    completedIntake: [],
  },
  "ongoing-control": {
    stage: "Readiness Control",
    title: "Your Readiness Control subscription is confirmed.",
    dashboardCta: "Open control requirements",
    completedEvidence: ["customerOwnershipVerified"],
    completedIntake: [],
  },
};
          ...
          <div className={CENDORQ_EXPERIENCE_SYSTEM.glassPanel}>
            <div className="overflow-hidden rounded-[1.85rem] border border-slate-200 bg-white sm:rounded-[2.35rem]">
              <div className="border-b border-slate-200 bg-[linear-gradient(135deg,#020617,#172554_62%,#083344)] p-6 text-white sm:p-8">
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-cyan-200">Resolved journey state</p>
                <h2 className="mt-4 text-4xl font-semibold tracking-[-0.06em] text-white sm:text-5xl">{context.stage}</h2>
                ...
                <div className="mt-5 inline-flex rounded-full border border-cyan-200/30 bg-cyan-200/10 px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-cyan-100">{humanize(journey.fulfillmentState)}</div>
              </div>
