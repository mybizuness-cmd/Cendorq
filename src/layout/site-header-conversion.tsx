...
const CTA_CLASS =
  "hidden min-[390px]:inline-flex min-h-10 shrink-0 items-center justify-center rounded-2xl border border-cyan-300/75 bg-[linear-gradient(135deg,rgba(255,59,147,0.18)_0%,rgba(0,217,255,0.10)_100%)] px-3 py-2 text-xs font-black text-white shadow-[0_0_28px_rgba(0,217,255,0.22),inset_0_1px_0_rgba(255,255,255,0.10)] transition hover:-translate-y-0.5 hover:border-fuchsia-400/80 hover:shadow-[0_0_34px_rgba(255,59,147,0.24)] focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 focus-visible:ring-offset-2 focus-visible:ring-offset-[#020713] sm:min-h-12 sm:px-5 sm:text-sm";

const NAV_LINK_BASE =
  "inline-flex min-h-9 flex-1 shrink items-center justify-center rounded-full px-2 py-2 text-[11px] font-black tracking-[-0.01em] text-white/90 transition hover:bg-white/8 hover:text-white focus:outline-none focus-visible:bg-white/10 focus-visible:text-white focus-visible:ring-2 focus-visible:ring-cyan-300 sm:min-h-10 sm:flex-none sm:px-4 sm:text-sm";
...
export async function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full overflow-visible border-b border-cyan-300/10 bg-[#020713]/94 text-white shadow-[0_14px_44px_rgba(0,0,0,0.26)] backdrop-blur-2xl">
      <div className="mx-auto flex min-h-[3.75rem] w-full max-w-[94rem] items-center justify-between gap-1.5 px-2 sm:min-h-[4.5rem] sm:gap-4 sm:px-6">
        ...
        <nav aria-label="Primary navigation" className="flex min-w-0 flex-1 items-center justify-center gap-0.5 overflow-hidden rounded-full border border-cyan-300/10 bg-[#020713]/20 px-1 py-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:gap-1.5 sm:px-1.5 lg:max-w-xl">
        ...
        <Link href={isRememberedCustomer ? session.safeReturnTo : "/free-check"} className={CTA_CLASS}>
          ...
          <span aria-hidden="true" className="ml-2 hidden text-cyan-300 sm:inline">→</span>
        </Link>
      </div>
...
function AccountMenu({ dashboardHref }: { dashboardHref: string }) {
  return (
    <details className="group relative z-50 hidden sm:block">
      ...
      <div className="absolute right-0 z-50 mt-2 w-[min(14rem,calc(100vw-1.5rem))] rounded-2xl border border-cyan-300/18 bg-[#061321] p-2 shadow-[0_18px_55px_rgba(0,0,0,0.42)]">
...
function BrandMark() {
  return (
    <span className="relative inline-flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-cyan-300/22 bg-[#020713] shadow-[0_0_30px_rgba(0,217,255,0.20)] sm:h-11 sm:w-11">
      <span className="absolute inset-0 bg-[radial-gradient(circle_at_30%_15%,rgba(255,59,147,0.72),transparent_42%),radial-gradient(circle_at_75%_80%,rgba(0,217,255,0.76),transparent_45%)]" aria-hidden="true" />
      <span className="relative flex items-center gap-[3px]">
        <span className="h-4 w-1.5 rounded-full bg-[#00d9ff]" />
        <span className="h-6 w-1.5 rounded-full bg-white" />
        <span className="h-4 w-1.5 rounded-full bg-[#ff3b93]" />
      </span>
    </span>
  );
}
