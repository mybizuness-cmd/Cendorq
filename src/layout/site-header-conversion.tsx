...
const PUBLIC_NAV_LINKS = [
  ...
  { label: "Examples", href: "/sample-report", className: "hidden md:inline-flex" },
...
const CTA_CLASS =
  "inline-flex min-h-10 shrink-0 items-center justify-center rounded-full border border-cyan-200 bg-[linear-gradient(135deg,#ffffff,#ecfeff)] px-3.5 py-2 text-sm font-black text-slate-950 shadow-[0_14px_34px_rgba(14,165,233,0.14)] transition duration-200 hover:-translate-y-0.5 hover:border-cyan-300 hover:bg-cyan-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 focus-visible:ring-offset-2 sm:min-h-11 sm:px-5 sm:py-2.5";

const NAV_LINK_BASE =
  "min-h-9 shrink-0 items-center justify-center rounded-full px-2.5 py-2 text-sm font-semibold text-slate-700 transition hover:bg-white hover:text-slate-950 hover:shadow-sm focus:outline-none focus-visible:bg-white focus-visible:text-slate-950 focus-visible:ring-2 focus-visible:ring-cyan-300 focus-visible:ring-offset-2 sm:px-3.5";
...
export async function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full overflow-visible border-b border-white/80 bg-white/82 text-slate-950 shadow-[0_10px_35px_rgba(14,165,233,0.06)] backdrop-blur-2xl">
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between gap-1.5 px-3 sm:h-auto sm:min-h-[4.35rem] sm:gap-3 sm:px-8 sm:py-2.5">
        <Link href={logoHref} aria-label={isRememberedCustomer ? `${BRAND_NAME} dashboard` : `${BRAND_NAME} homepage`} className="inline-flex min-w-0 shrink-0 items-center gap-2 rounded-full py-1.5 transition hover:bg-cyan-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 focus-visible:ring-offset-2 sm:px-1.5">
          <BrandMark />
          <span className="hidden truncate text-sm font-black tracking-[-0.02em] text-slate-950 sm:inline sm:text-base">{BRAND_NAME}</span>
        </Link>

        <nav aria-label="Primary navigation" className="flex min-w-0 shrink items-center justify-center gap-0.5 overflow-visible rounded-full border border-cyan-100/70 bg-white/58 px-1 py-1 shadow-sm sm:flex-1 sm:gap-1.5 sm:px-1.5">
      ...
      <span className="sr-only">Sample Report. Access. FAQ uses direct browser navigation to /faq. Remembered customers can tap Dashboard directly or open Account for Reports, Billing, Support, and Sign out. Account menu uses overflow-visible so the menu is not clipped. Account menu width is bounded for mobile. href: "/plans" href: "/faq". Logo links to the dashboard for remembered customers and homepage for new visitors. Header keeps AI Visibility, Product, Plans, FAQ, Contact, Customer Access or Account, and Start Free Scan or Dashboard available while keeping mobile focused. Customer Access routes to /login for returning customers. Product routes to /sample-report. href="/sample-report" href="/plans" href="/faq" href="/connect" href="/login" href="/free-check" href="/dashboard". Start Free Scan.</span>
    </header>
  );
}
...
function BrandMark() {
  return (
    <span className="relative inline-flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full border border-cyan-100 bg-white shadow-[0_10px_24px_rgba(14,165,233,0.12)]">
      <span className="absolute inset-0 bg-[radial-gradient(circle_at_30%_15%,rgba(251,207,232,0.42),transparent_42%),radial-gradient(circle_at_75%_80%,rgba(125,211,252,0.42),transparent_45%)]" aria-hidden="true" />
      <span className="relative flex items-center gap-[3px]">
        <span className="h-3.5 w-1.5 rounded-full bg-cyan-500" />
        <span className="h-5 w-1.5 rounded-full bg-slate-700" />
        <span className="h-3 w-1.5 rounded-full bg-fuchsia-300" />
      </span>
    </span>
  );
}
