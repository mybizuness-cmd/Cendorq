...
The first touch must immediately feel different from a normal agency, consultant, or automated audit tool.
...
3. Cendorq internal evidence
   - intake status
   - report history
   - support state
   - billing entitlement state
   - dashboard handoff state
   - owner-approved configuration state
...
5. Confidence and limits
   ...
   - medium confidence when evidence is partial but directionally consistent
   - low confidence when evidence is thin, conflicting, stale, or customer-only
   - explicit limitations when evidence cannot be safely gathered

### Report sections that must stay separated

Every substantial report should keep these sections distinct:
...
1. Intake capture
   - collect minimum useful business context
   - do not collect passwords, private keys, card data, tokens, or unrelated raw evidence
   - keep incomplete input as pending, not final truth

2. Evidence gathering
   - gather safe public evidence when available
   - check public website and platform posture
   - check relevant category/market context
   - identify missing evidence instead of guessing

3. Evidence conflict pass
   - compare customer claims against external evidence
   - mark contradictions as evidence conflicts
...
Plans must not be generic packages. They must behave like stage-specific operating paths.
...
Cendorq should improve as more businesses are studied, without turning customer-specific data into unsafe claims.

The learning loop should identify:
...
Owner command is highest authority. Release captain is execution command and final validator.

Chief agents and sub-agents can:
...
Before public launch, owner should confirm:

1. Auth provider configuration is real and production-ready.
2. Payment mapping is real and owner-approved.
3. Protected runtime configuration is ready without browser exposure.
4. Launch contact identity is complete.
5. Support identity is complete.
6. Production smoke target is configured.
7. Rollback plan exists.
8. Audit plan exists.
9. Vercel is green for release PR.
10. `validate:routes` passes through the central route-chain orchestrator.
11. No launch blocker is active.

After public launch, owner should review:

- Free Scan submissions
- report conversion quality
- support volume and support types
- billing friction
...
1. Durable owner evidence persistence
2. Owner approval workflow storage
3. Release-captain review event storage
4. Report evidence record persistence
...
- refine plan fit
- update conversion copy based on evidence
